...
class TCM(CompressionModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.m_down1 = [
            ConvTransBlock(
                self.dim, self.dim, self.head_dim[0], self.window_size, dpr[i + begin], "W" if not i % 2 else "SW"
            )
            for i in range(config[0])
        ] + [ResidualBlockWithStride(2 * N, 2 * N, stride=2)]
        self.m_down2 = [
            ConvTransBlock(
                self.dim, self.dim, self.head_dim[1], self.window_size, dpr[i + begin], "W" if not i % 2 else "SW"
            )
            for i in range(config[1])
        ] + [ResidualBlockWithStride(2 * N, 2 * N, stride=2)]
        self.m_down3 = [
            ConvTransBlock(
                self.dim, self.dim, self.head_dim[2], self.window_size, dpr[i + begin], "W" if not i % 2 else "SW"
            )
        ...
        self.m_up1 = [
            ConvTransBlock(
                self.dim, self.dim, self.head_dim[3], self.window_size, dpr[i + begin], "W" if not i % 2 else "SW"
            )
            for i in range(config[3])
        ] + [ResidualBlockUpsample(2 * N, 2 * N, 2)]
        self.m_up2 = [
            ConvTransBlock(
                self.dim, self.dim, self.head_dim[4], self.window_size, dpr[i + begin], "W" if not i % 2 else "SW"
            )
            for i in range(config[4])
        ] + [ResidualBlockUpsample(2 * N, 2 * N, 2)]
        self.m_up3 = [
            ConvTransBlock(
                self.dim, self.dim, self.head_dim[5], self.window_size, dpr[i + begin], "W" if not i % 2 else "SW"
            )
        ...
        self.ha_down1 = [ConvTransBlock(N, N, 32, 4, 0, "W" if not i % 2 else "SW") for i in range(config[0])] + [
            conv3x3(2 * N, 192, stride=2)
        ]

        self.h_a = nn.Sequential(*[ResidualBlockWithStride(320, 2 * N, 2)] + self.ha_down1)

        self.hs_up1 = [ConvTransBlock(N, N, 32, 4, 0, "W" if not i % 2 else "SW") for i in range(config[3])] + [
            subpel_conv3x3(2 * N, 320, 2)
        ]

        self.h_mean_s = nn.Sequential(*[ResidualBlockUpsample(192, 2 * N, 2)] + self.hs_up1)

        self.hs_up2 = [ConvTransBlock(N, N, 32, 4, 0, "W" if not i % 2 else "SW") for i in range(config[3])] + [
            subpel_conv3x3(2 * N, 320, 2)
        ]
