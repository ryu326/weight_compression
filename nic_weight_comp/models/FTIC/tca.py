...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, slices=12, act_layer=nn.GELU()):
        super().__init__()
...
class TCA_Block(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = Mlp(in_features=dim, slices=slices, hidden_features=mlp_hidden_dim, act_layer=act_layer)
    ...
    def generate_mask(self, dim):
        ...
        for i in range(self.slices - 1):
            attn_mask[
                :,
                : (i + 1) * heads_dim // self.slices,
                (i + 1) * heads_dim // self.slices : (i + 2) * heads_dim // self.slices,
            ] = 1
        return attn_mask
...
class TCA(nn.Module):
    def __init__(self, dim=192, depth=4, ratio=4, slices=12, drop_path_rate=0.3):
        ...
        for i_layer in range(self.num_layers):
            layer = TCA_Block(dim=dim * ratio, slices=slices)
...
class TCA_EntropyModel(nn.Module):
    ...
    def forward(self, hyper, y):
        ...
        out = (
            self.entropy_parameters_net(torch.cat((out1, hyper1), 2).view(B, C * (self.ratio + 2), H, W))
            .view(B, C, 3, H, W)
            .contiguous()
        )  # B 2C H W
...
if __name__ == "__main__":
    ...
    for i in range(slices):
        ...
        print(i)
        print(
            (means1 - means2).abs().mean(-1).mean(-1)[0][:k].mean(),
            (means1 - means2).abs().mean(-1).mean(-1)[0][k:].mean(),
        )
        print(
            (scales1 - scales2).abs().mean(-1).mean(-1)[0][:k].mean(),
            (scales1 - scales2).abs().mean(-1).mean(-1)[0][k:].mean(),
        )
