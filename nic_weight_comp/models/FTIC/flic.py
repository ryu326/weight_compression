...
class FrequencyAwareTransFormer(CompressionModel):
    def __init__(
        ...
        **kwargs
        ...
        self.m_down1 = [
            FAT_Block(N1, self.head_dim[0], self.window_size, self.window_size * 2, dpr[i], "W" if not i % 2 else "SW")
            for i in range(config[0])
        ] + [ResidualBlockWithStride(N1, N2, stride=2)]

        self.m_down2 = [
            FAT_Block(
                N2, self.head_dim[1], self.window_size, self.window_size * 2, dpr[i + 2], "W" if not i % 2 else "SW"
            )
        ...
        self.m_down3 = [
            FAT_Block(
                N3, self.head_dim[2], self.window_size, self.window_size * 2, dpr[i + 4], "W" if not i % 2 else "SW"
            )
        ...
        dpr = dpr[::-1]
        self.m_up1 = [
            FAT_Block(N3, self.head_dim[3], self.window_size, self.window_size * 2, dpr[i], "W" if not i % 2 else "SW")
            for i in range(config[3])
        ] + [ResidualBlockUpsample(N3, N2, 2)]
        self.m_up2 = [
            FAT_Block(
                N2, self.head_dim[4], self.window_size, self.window_size * 2, dpr[i + 2], "W" if not i % 2 else "SW"
            )
            for i in range(config[4])
        ] + [ResidualBlockUpsample(N2, N1, 2)]
        self.m_up3 = [
            FAT_Block(
                N1, self.head_dim[5], self.window_size, self.window_size * 2, dpr[i + 4], "W" if not i % 2 else "SW"
            )
        ...
        self.ha_down1 = [FAT_Block(N * 2, 32, 2, 4, 0, "W" if not i % 2 else "SW", True) for i in range(config[0])] + [
            conv3x3(2 * N, 192, stride=2)
        ]

        self.h_a = nn.Sequential(*[ResidualBlockWithStride(320, N3, 2)] + self.ha_down1)

        self.hs_up1 = [FAT_Block(N * 2, 32, 2, 4, 0, "W" if not i % 2 else "SW", True) for i in range(config[3])] + [
            subpel_conv3x3(2 * N, 320, 2)
        ]

        self.h_mean_s = nn.Sequential(*[ResidualBlockUpsample(192, 2 * N, 2)] + self.hs_up1)

        self.hs_up2 = [FAT_Block(N * 2, 32, 2, 4, 0, "W" if not i % 2 else "SW", True) for i in range(config[3])] + [
            subpel_conv3x3(2 * N, 320, 2)
        ]
    ...
    def forward(self, x):
        ...
        return {
            "x": x,
            "x_hat": x_hat,
            "x_low_freq": x_low_freq,
            "x_high_freq": x_high_freq,
            "x_hat_low_freq": x_hat_low_freq,
            "x_hat_high_freq": x_hat_high_freq,
                ...
                for w_idx in range(y_shape[1]):
                    for c_idx in range(channel_per_slices):
                        ...
                        pmf = self._likelihood(
                            torch.tensor(samples).to(x.device),
                            scale[0][c_idx][h_idx][w_idx],
                            means=mu[0][c_idx][h_idx][w_idx] + minmax,
                        )
                        pmf = lower_bound(pmf)
            ...
            for h_idx in range(y_shape[0]):
                for w_idx in range(y_shape[1]):
                    for c_idx in range(channel_per_slices):
                        ...
                        pmf = self._likelihood(
                            torch.tensor(samples).to(z_hat.device),
                            scale[0][c_idx][h_idx][w_idx],
                            means=mu[0][c_idx][h_idx][w_idx] + minmax,
                        )
                        pmf = lower_bound(pmf)
