...
def _get_center_distance(size: Tuple[int], device: str = "cpu") -> Tensor:
    ...
    """
    m, n = size
    i_ind = torch.tile(torch.tensor([[[i]] for i in range(m)], device=device), dims=[1, n, 1]).float()  # [m, n, 1]
    j_ind = torch.tile(torch.tensor([[[i] for i in range(n)]], device=device), dims=[m, 1, 1]).float()  # [m, n, 1]
...
def parse_args(argv):
    ...
    parser.add_argument("--dist_port", type=int, default=6006, required=True, help="dist_port(default: %(default)s)")
    parser.add_argument(
        "--iter",
        default=200000,
        type=int,
    ...
    parser.add_argument("--radius_denominator", type=int, default=8)
    parser.add_argument("--dataset_dir", type=str, default="../Wparam_dataset")
