...
from datasets_weight_vector_row_idx_calib_mag import get_dataset_row_16_calib, LayerInputs
from datasets_weight_vector_col_idx_calib_mag import get_dataset_col_16_calib
from loss import *
from models import VQ_SEEDLM, VQVAE, VQVAE_IDX
...
def test(total_iter, test_dataset, model, save_path, logger, mse_func, node_rank=0):
    ...
    for idx, data in enumerate(test_dataset):
        ...
        weight = weight.unsqueeze(0).to(device)  # [1, 512, 512] 같음
        tensor_block_idx = tensor_block_idx.unsqueeze(0).to(device) 
        
        out = model(weight, tensor_block_idx)

        x_hat = out["x_hat"].clone().detach()

        mse = mse_func(weight, x_hat).item()
...
def main(opts):
    ...
    if opts.block_direction == 'row':
        train_dataset, test_dataset, train_std, test_std = get_dataset_row_16_calib()
    elif opts.block_direction == 'col':
        train_dataset, test_dataset, train_std, test_std = get_dataset_col_16_calib()
    ...
    elif opts.architecture == "vqvae_idx":
        assert opts.K is not None
        net = VQVAE_IDX(
            input_size=opts.input_size,
            dim_encoder=opts.dim_encoder,
    ...
    while 1:
        ...
        for i, (data) in enumerate(train_dataloader):
            ...
            with torch.autograd.detect_anomaly():
                ...
                out_net = net(weight, tensor_block_idx)
                ...
                recon_loss = criterion(out_net["x"], out_net["x_hat"], data['input_block'].to(device))
...
def ddp_or_single_process(argvs):
    ...
    folder_name = f"per_{opts.block_direction}_16_calib"
    folder_name += f"/bpp{opts.bpp}_size{opts.input_size}_{opts.loss}_ne{opts.n_embeddings}_de{opts.dim_embeddings}_K{opts.K}_P{opts.P}_encdim{opts.dim_encoder}"
