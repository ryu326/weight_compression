##  int8, shared exponetial으로 coefficient 표현하기
## 폐기
...
class VQ_SEEDLM(nn.Module):

    def __init__(
        self, input_size, dim_encoder, n_resblock, n_embeddings, P, beta, scale, shift
    ...
    def forward(self, x, verbose=False):
        ...
        coefficient = z[:, : self.P]
        # coefficient = coefficient.to(dtype=torch.float16)

        # import ipdb; ipdb.set_trace()
        e = torch.floor(torch.log2(torch.abs(coefficient) + 1e-8))
        e = torch.max(e, dim=1, keepdim=True).values

        coefficient = (coefficient / torch.exp2(e)).to(dtype=torch.int8)

        z_e = z[:, self.P :].contiguous().view(-1, self.P, self.dim_embeddings)
        ...
        coefficient = (coefficient * torch.exp2(e)).to(dtype=torch.float32)
        x_hat = torch.matmul(z_q.permute(0, 2, 1).contiguous(), coefficient.unsqueeze(-1))
