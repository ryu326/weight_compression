...
class VQ_SEEDLM(nn.Module):
    ...
    def __init__(
        ...
        self.dim_embeddings = input_size * P  ## V3
    ...
    def forward(self, x, verbose=False):
        ...
        coefficient = coefficient.to(dtype=torch.bfloat16)

        z_e = z[:, self.P :]

        embedding_loss, z_q, perplexity, min_encodings, min_encoding_indices = self.vector_quantization(z_e)

        ## (b, inputsize, P) @ (b, P, 1) --> (b, inputsize, 1)
        coefficient = coefficient.to(dtype=torch.float32)
        x_hat = torch.matmul(z_q.contiguous().view(-1, self.input_size, self.P), coefficient.unsqueeze(-1))
