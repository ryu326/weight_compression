...
def main(opts):
    wandb.init(project="NN-based SEEDLM_v2", name="nn_seedlm")
    ...
    net = VQ_SEEDLM(input_size = opts.input_size, 
    ...
    mse_func = nn.MSELoss()
    folder_name = '/'.join(opts.dataset_path.split('/')[-2:]) + f'/size{opts.input_size}_ne{opts.n_embeddings}_de{opts.dim_encoder}_P{opts.P}_batch_size{opts.batch_size}_total_iter{opts.iter}_lr{opts.learning_rate}_seed{opts.seed}'
        ...
        for i, (weight) in enumerate(train_dataloader):
            ...
            # if (total_iter % 50000 == 0):
                ...
                with torch.no_grad() :
                    ...
                    net_eval =  VQ_SEEDLM(input_size = opts.input_size, 
                                    dim_encoder = opts.dim_encoder, 
                                    P = opts.P, n_embeddings = opts.n_embeddings, n_resblock = opts.n_resblock, 
                                    beta = opts.vq_beta, 
                                    scale = train_dataset.std.to(device),
