...
class Model(tf.keras.Model):
    ...
    def train_step(self, x):
        ...
        # wandb.log({'learning_rate': tf.keras.backend.get_value(self.optimizer.lr)})
        # wandb.log({m.name: m.result() for m in [self.loss, self.rate, self.mse]})
        # wandb.log({'loss': loss.numpy(), 'rate':rate.numpy(), 'mse': mse.numpy()})
    ...
    def test_step(self, x):
        ...
        self.mse.update_state(mse)
        # wandb.log({m.name: m.result() for m in [self.loss, self.rate, self.mse]})
        # wandb.log({'val_loss': loss.numpy(), 'val_rate':rate.numpy(), 'val_mse': mse.numpy()})
        return {m.name: m.result() for m in [self.loss, self.rate, self.mse]}
...
def train(args):
    ...
    from utils_wp import \
        get_wp_tfrecord  # get_custom_wp_dataset, get_wp_datasets,
    ...
    if args.use_wp_dataset:
        if args.dataset.endswith(".npy") or args.dataset.endswith(".npz"):
            ...
            #     train_dataset, validation_dataset = get_wp_datasets(args.dataset, args.batchsize, repeat = args.repeat)
            pass
        elif args.dataset.endswith(".tfrecord"):
            ...
            validation_dataset = get_wp_tfrecord(
                "val",
                args.dataset.replace("train", "val"),
                args,
                num_data=args.max_validation_steps,
                repeat=args.repeat,
            )
        else:
            # train_dataset = get_custom_wp_dataset('train', args.dataset, args, num_data=args.num_data, repeat = args.repeat)
            # validation_dataset = get_custom_wp_dataset('val', args.dataset, args, num_data=args.max_validation_steps, repeat = args.repeat)
            pass
