...
def _rotation_2d(degrees):
    ...
    rotation = tf.linalg.LinearOperatorFullMatrix(rotation, is_non_singular=True, is_square=True)
    return rotation
...
def get_banana():
    return tfpd.TransformedDistribution(
        tfpd.Independent(tfpd.Normal(loc=[0, 0], scale=[3, 0.5]), 1),
        tfpb.Invert(
            tfpb.Chain(
                [
                    tfpb.RealNVP(num_masked=1, shift_and_log_scale_fn=lambda x, _: (0.1 * x**2, None)),
...
def get_nd_banana(n, batchsize, seed=0):
    ...
    embedder = make_mlp([n], activation=tf.nn.softplus, name="banana_embedder", input_shape=[2], dtype=source.dtype)
