...
from utils import get_custom_dataset, read_png, write_png
...
def parse_args(argv, add_model_specific_args=None):
    ...
    train_cmd.add_argument("--batchsize", type=int, default=8, help="Batch size for training and validation.")
    ...
    train_cmd.add_argument("--lr", type=float, default=1e-4, help="Initial lr to configure the optimizer with.")
    ...
    train_cmd.add_argument(
        "--warmup", type=int, default=100, help="Don't start decaying lr until the number of epochs hits `warmup`."
    )
    train_cmd.add_argument(
        "--validation_steps",
        type=int,
    ...
    decompress_cmd = subparsers.add_parser(
        "decompress",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        description="Reads a TFCI file, reconstructs the image, and writes back " "a PNG file.",
    )
    ...
    for cmd, ext in ((compress_cmd, ".tfci"), (decompress_cmd, ".png")):
        ...
        cmd.add_argument(
            "output_file",
            nargs="?",
            help=f"Output filename (optional). If not provided, appends '{ext}' to " f"the input filename.",
        )
    ...
    for cmd, mode in zip((train_cmd, eval_cmd), ("training", "evaluation")):
        ...
        cmd.add_argument(
            "--data_dim", type=int, default=None, help="Intrinsic data dimension for custom data generator."
        )
...
def train(args, create_model, get_runname):
    ...
    if args.dataset is not None:
        ...
        if args.dataset in configs.biggan_class_names_to_ids:  # train on GAN
            ...
            post_process_fun = lambda x: (x + 1.0) * 127.5
    ...
    if train_dataset is not None:
        ...
        validation_dataset = tf.data.Dataset.from_tensor_slices(
            [x for (i, x) in zip(range(args.validation_steps), train_data_generator)]
        )
        validation_data = validation_dataset.cache()
...
def evaluate(args, create_model, get_runname):
    ...
    for x in dataset:
        ...
        if args.bits:
            ...
            if args.no_cast_xhat:
                ...
                x_hat = tf.clip_by_value(x_hat, 0.0, max_pxl_val)
            else:
        ...
        batch_res["msssim_db"] = -10.0 * tf.math.log(1 - batch_res["msssim"]) / tf.math.log(10.0)
    ...
    save_path = os.path.join(save_dir, f"{prefix}-{runname}{dataset_str}" f"{avg_rate_str}{avg_distortion_str}.npz")
