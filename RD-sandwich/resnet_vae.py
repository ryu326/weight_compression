...
from utils import (diag_normal_from_features, reshape_spatially_as,
                   transform_scale_indexes)
...
class ChannelwiseARTransform(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        if num_slices > input_num_channels:
            print(
                f"Warning: num_slices ({num_slices}) > input_num_channels ({input_num_channels}), "
        ...
        if slice_depth * num_slices != input_num_channels:
            raise ValueError("Slices do not evenly divide latent depth (%d / %d)" % (input_num_channels, num_slices))
        ...
        self.transforms = [
            tf.keras.Sequential(
                [
                    ...
                    conv(1 * slice_depth, (5, 5), name="layer_2", activation=output_activation),
                ]
            )
...
def enc_conv(filters, kernel_support, stride, name=None, activation=tf.nn.leaky_relu, kernel_parameter="rdft"):
    return tfc.SignalConv2D(
        filters,
        kernel_support,
        name=name,
        corr=True,
...
def dec_conv(filters, kernel_support, stride, name=None, activation=tf.nn.leaky_relu, kernel_parameter="rdft"):
    return tfc.SignalConv2D(
        filters,
        kernel_support,
        name=name,
...
class EncoderBlock(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.conv1 = enc_conv(
            self.filters, self.kernel_support, self.stride, kernel_parameter=self.kernel_parameter, name="conv1"
        )
        self.conv2 = enc_conv(
            self.filters,
            self.kernel_support,
            1,
            activation=self.conv2a,
        ...
        if input_channels == self.filters and self.stride == 1:
            ...
            self.shortcut = enc_conv(
                self.filters, (1, 1), self.stride, activation=None, name="shortcut", kernel_parameter="variable"
            )
...
class DecoderBlock(tf.keras.layers.Layer):
    def __init__(self, filters, kernel_support=(3, 3), stride=1, name=None, kernel_parameter="rdft", img_output=False):
        """
        ...
        if not img_output:
            ...
            assert filters == 3
            self.conv = dec_conv(
                filters, kernel_support, stride, activation=None, kernel_parameter=kernel_parameter, name="conv"
            )
...
class LatentBlock(tf.keras.Model):
    def __init__(
        ...
        **kwargs,
        ...
        self.td_inf_net = dec_conv(
            2 * latent_channels, kernel_support, stride=1, name=f"{name}-td_inf_net", kernel_parameter=kernel_parameter
        )

        # first chunk will be top-down deterministic feature, second/third chunks are prior mean/scale parameters
        self.gen_net = dec_conv(
            latent_channels * 3,
            kernel_support,
            stride=1,
            activation=None,
        ...
        self.t_adaptor = dec_conv(
            feature_channels, (3, 3), stride=1, name=f"{name}-t_adaptor", kernel_parameter=kernel_parameter
        )
        self.ar_prior = ar_prior
        if ar_prior:
            ar_num_slices = kwargs["ar_num_slices"]
            max_support_ratio = kwargs.get("max_support_ratio", 0.5)
            self.ar_prior_shift_transform = ChannelwiseARTransform(
                input_num_channels=latent_channels,
                num_slices=ar_num_slices,
                max_support_ratio=max_support_ratio,
                cond_first_slices=True,
            )
            self.ar_prior_scale_transform = ChannelwiseARTransform(
                input_num_channels=latent_channels,
                num_slices=ar_num_slices,
                max_support_ratio=max_support_ratio,
                cond_first_slices=True,
    ...
    def call(self, b, t, training):
        ...
        if self.ar_prior:
            ...
            scale = tf.nn.sigmoid(
                logit_offset + self.ar_prior_scale_transform(z, return_slices=False)
            )  # sigma_t in Eq (13) of IAF
            epsilon = scale * z + (1 - scale) * shift  # everything done elementwise
            log_det_J = tf.reduce_sum(
                tf.math.log(scale), axis=reduce_axes
            )  # [batch_size]; this is log det of the 'inverse AR transform': z->\epsilon
            ...
            bits = tf.reduce_sum(bits, axis=reduce_axes) / tf.math.log(
                tf.constant(2, dtype=p_loc.dtype)
            )  # sum along all axes except batch
...
class Model(tf.keras.Model):
    ...
    def __init__(self, **kwargs):
        ...
        for bu_idx in range(self.num_levels):
            ...
            if td_idx == 0:  # topmost encoder computes parameters of q(z0)
                if num_filters != 2 * z0_channels:
                    print(
                        f"Overriding the last encoder num_filters to 2 times z0 channels = {z0_channels}"
                        f"for q(z0) mean,scale"
                    )
                enc_block = EncoderBlock(
                    filters=2 * z0_channels,
                    stride=stride,
                    name=enc_block_name,
                    output_activation=None,
                    scale_img=scale_img,
                ...
                enc_block = EncoderBlock(filters=num_filters, stride=stride, name=enc_block_name, scale_img=scale_img)
            self.set_enc_block(bu_idx, enc_block)

            dec_block_name = f"dec_block_{td_idx}"
            if bu_idx == 0:  # the bottom-most decoder
                dec_block = DecoderBlock(filters=3, stride=stride, name=dec_block_name, img_output=scale_img)
            else:
                ...
                dec_block = DecoderBlock(
                    filters=prev_enc_num_filters, stride=stride, name=dec_block_name, img_output=scale_img
                )
        ...
        if not self.flat_z0:  # fully convolutional
            ...
            self.pz0_base_dist = tfd.MultivariateNormalDiag(
                loc=tf.zeros([self.z0_flat_dim], dtype=self.dtype),
            ...
            self._maf_mades = [
                tfb.AutoregressiveNetwork(params=2, activation=tf.nn.leaky_relu, hidden_units=self.maf_units)
    ...
    def add_model_specific_args(parser):
        ...
        parser.add_argument("--img_dim", type=int, default=None, help="Input img dim for non-fully convolutional arch.")
        ...
        parser.add_argument(
            "--downsample_factor", type=int, default=2, help="The downsampling factor ('stride') of every EncoderBlock"
        )
        parser.add_argument(
            "--latent_channels",
        ...
        parser.add_argument(
            "--ar_slices", type=int, default=0, help="Number of channel-wise slices for the channel-wise MAF prior."
        )
        parser.add_argument(
            "--ar_max_support_ratio",
            type=float,
            default=0.5,
        ...
        parser.add_argument("--maf_stacks", type=int, default=3, help="Num of MAF transforms.")
    ...
    def call(self, x, training=True):
        ...
        for i in range(self.num_levels):
            ...
            if i == 0:  # topmost latent
                ...
                if self.flat_z0:  # flatten z0 to evaluate under flat prior
                    ...
                    orig_z0_shape = tf.constant(
                        [-1, self.z0_spatial_dim, self.z0_spatial_dim, self.z0_channels], dtype="int32"
                    )
        ...
        return dict(
            loss=loss, bpp=bpp, mse=mse, mses=mses, bits=bits, x_hat=x_hat, psnr=psnr, bits_per_latent=bits_per_latent
        )
...
def get_runname(args):
    ...
    runname = config_dict_to_str(
        vars(args),
        record_keys=("lmbda", "num_filters", "latent_channels", "ar_prior_levels", "ar_slices"),
        prefix=model_name,
        use_abbr=True,
    )
    return runname
