...
def gen_dataset(dataset_spec: str, data_dim: int, batchsize: int, dtype="float32", **kwargs):
    ...
    if dataset_spec == "gaussian":
        if kwargs.get("gparams_path", None):
            ## ryu
            kwargs.get("gaussian_scale", None)
            gparams = np.load(kwargs["gparams_path"])
            loc = gparams["loc"].astype(dtype)
            scale = gparams["scale"].astype(dtype) * kwargs["gaussian_scale"]
        else:
            loc = np.zeros(data_dim, dtype=dtype)
            # scale = np.ones(data_dim, dtype=dtype)
            ## ryu
            kwargs.get("gaussian_scale", None)
            scale = np.ones(data_dim, dtype=dtype) * kwargs["gaussian_scale"]
        source = tfd.Normal(loc=loc, scale=scale)
        map_sample_fun = lambda _: source.sample(batchsize)
    elif dataset_spec == "uniform":
        source = tfd.Uniform(low=-tf.sqrt(3.0), high=tf.sqrt(3.0))
        map_sample_fun = lambda _: source.sample([batchsize, data_dim])
...
def get_lr_scheduler(learning_rate, epochs, decay_factor=0.1, warmup_epochs=0):
    ...
    def scheduler(epoch, lr):
        ...
        if epoch < warmup_epochs:
            return learning_rate * 10.0 ** (epoch - warmup_epochs)
        if epoch < 1 / 2 * epochs:
...
def parse_args(argv):
    ...
    ## ryu
    parser.add_argument("--gaussian_scale", type=float, default=1.0)
