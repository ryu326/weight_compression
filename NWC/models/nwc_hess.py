...
class SimpleVAECompressionModel_hess(CompressionModel):
    ...
    .. code-block:: none
        ...
        x ──►──┤g_a├──►──┤ lc ├───►───┤g_s├──►── x_hat
               └───┘     └────┘       └───┘
    """
    ## M: enc out dim, R: # eigenvector, m*M: bottlencek 차원
    def __init__(self, input_size, dim_encoder, n_resblock, M, R, m, scale, shift):
        super().__init__()
        ...
        self.g_a_hess =  nn.Sequential(
            nn.Linear(input_size, dim_encoder),
            ResidualStack(dim_encoder, n_resblock),
            nn.Linear(dim_encoder, M),
        )
        
        self.comb =  nn.Linear(R+1, m)

        self.g_s = nn.Sequential(
            nn.Linear(M*m, dim_encoder),
            ResidualStack(dim_encoder, n_resblock),
            nn.Linear(dim_encoder, input_size),
        )
        
        self.entropy_bottleneck = EntropyBottleneck(M*m)
    ...
    def forward(self, data):
        ...
        # import ipdb; ipdb.set_trace()
        y = y.unsqueeze(1)
        y = torch.cat([y, h_y], dim=1) # (B, 11, 128, 16)
        
        perm = list(range(y.dim()))
        perm[-1], perm[1] = perm[1], perm[-1]
        
        y = y.permute(*perm).contiguous() # (B, 16, 128, 11)
        y = self.comb(y) # (B, 16, 128, 2)
        y = y.permute(0, 2, 1, 3).contiguous()  # (B, 128, 16, 2)
        y = y.contiguous().flatten(start_dim=-2) # (B, 128, 32)
