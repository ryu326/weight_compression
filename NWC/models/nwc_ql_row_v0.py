...
class NWC_ql_row(CompressionModel):
    """
    ...
    def __init__(
        ...
        *,  # keyword-only below
        ...
        self.register_buffer('shift', torch.zeros(1, dtype=torch.float32))
        ...
        self.quality_embedding = nn.Embedding(Q, dim_proj)
        self.cond_layers = nn.ModuleList([FiLMLayer(dim_proj, norm = norm) for _ in range(n_clayers)])

        # Decoder-side FiLM
        self.proj_y = nn.Linear(dim_proj, 1)
        self.cond_layers_dec = nn.ModuleList([FiLMLayer(dim_proj, norm = norm) for _ in range(n_clayers)])

        # Core encoder/decoder on flattened [B, L, dim_encoder]
        self.g_a = MLPEncoder(input_size*dim_proj, n_resblock, dim_encoder, M, norm)
        self.g_s = MLPEncoder(M, n_resblock, dim_encoder, input_size*dim_proj, norm)
    ...
    def _encode_conditioned(self, x: Tensor, q_level: Tensor) -> Tensor:
        """
        x: [B, L, I], s: [B, I]  -> returns y: [B, L, M]
        """
        x_p = self.proj_x(x.unsqueeze(-1))  # -> [B, L, I, P]
        q_embed = self.quality_embedding(q_level) # -> [B, I, P]
        q_embed = q_embed.unsqueeze(1)  # -> [B, 1, I, P]
        # print(x_p.shape, q_embed.shape)
        for layer in self.cond_layers:
            x_p = layer(x_p, q_embed)
        x_flat = torch.flatten(x_p, start_dim=-2)       # [B, L, I*P] == [B, L, dim_encoder]
        y = self.g_a(x_flat)                            # [B, L, M]
        return y

    def _decode_conditioned(self, y: Tensor, q_level: Tensor) -> Tensor:
        """
    ...
    def forward(self, data: Dict[str, Tensor], *, scale: Optional[Tensor] = None, shift: Optional[Tensor] = None):
        ...
        q_level = data['q_level'] # (B, I)
        # print(q_level.shape)
        # (Optional) PE
        if self.pe:
            x = self._apply_pe(x, data, self.pe_film_en)

        # Encode with FiLM
        y = self._encode_conditioned(x, q_level)  # [B, L, M]
        ...
        if self.use_hyper:
            ...
            x_hat = self._decode_conditioned(y_hat, q_level)             # [B, L, I]
    ...
    def compress(self, data: Dict[str, Tensor]):
        ...
        if self.pe:
            return {"strings": [y_strings], "shape": y_shape, "q_level": q_level, "depth": data["depth"], "ltype": data["ltype"]}
        else:
            return {"strings": [y_strings], "shape": y_shape, "q_level": q_level,}
            
    def decompress(self, enc_data: Dict[str, object]):
        strings = enc_data["strings"]
        shape = enc_data["shape"]
        q_level: Tensor = enc_data["q_level"]  # [B, L, I]
        ...
        x_hat = self._decode_conditioned(y_hat, q_level)
