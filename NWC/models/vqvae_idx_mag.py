...
class IdxEmbedding(nn.Module):
    def __init__(self, num_layers=32, hidden_dim=16, out_dim = 16):
        super().__init__()
        ...
        self.fc = nn.Linear(hidden_dim * 3, hidden_dim)   # 임베딩 + 숫자 입력
...
class VQVAE_IDX_MAG(nn.Module):
    ...
    def __init__(
        ...
        self.idx_cat = nn.Linear(self.dim_encoder_out + self.idx_embedding.out_dim, self.dim_encoder_out)

    def forward(self, data, verbose=False):
        
        x = data['weight_block']
        block_idx_tensor = data['tensor_block_idx']
        ac = torch.sqrt(data['input_block'])
        
        x_shift = (x - self.shift) / self.scale

        out_enc_w = self.w_encoder(x_shift)
        out_enc_ac = self.ac_encoder(ac)
        out_idx = self.idx_embedding(block_idx_tensor)
        
        z_e = torch.cat([out_enc_w * out_enc_ac, out_idx], dim=-1)
