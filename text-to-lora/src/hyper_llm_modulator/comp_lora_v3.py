...
class LowRankFiLM(nn.Module):
    def __init__(self, cond_dim, dim, rank=16, share_basis=True):
        super().__init__()
        self.rank = rank
        self.U = nn.Parameter(torch.empty(dim, rank)); nn.init.orthogonal_(self.U)
        self.head = nn.Sequential(nn.LayerNorm(cond_dim), nn.Linear(cond_dim, 2*rank))
        self.share_basis = share_basis
        if not share_basis:
            self.Ub = nn.Parameter(torch.empty(dim, rank)); nn.init.orthogonal_(self.Ub)
    def forward(self, h, cond):
        a, b = self.head(cond).chunk(2, dim=-1)      # [*, r], [*, r]
        Uβ = self.U if self.share_basis else self.Ub # [D, r]
        gamma = (a @ self.U.T)                       # [*, D]
        beta  = (b @ Uβ.T)                           # [*, D]
        return h * (1.0 + torch.tanh(gamma)) + beta

class LowRankResidual(nn.Module):
    def __init__(self, dim: int, rank: int, rezero: bool = True):
        super().__init__()
        self.U = nn.Linear(dim, rank, bias=False)
        self.V = nn.Linear(rank, dim, bias=False)
        self.alpha = nn.Parameter(torch.zeros(1)) if rezero else None
        self.log_diag = nn.Parameter(torch.zeros(dim))  # optional diag gain
    def forward(self, x):
        y = self.V(self.U(x))
        if self.alpha is not None:
            y = torch.tanh(self.alpha) * y
        x_scaled = x * (1.0 + torch.tanh(self.log_diag))  # cheap per-dim gate
        return x_scaled + y
    
class EncoderCoreV3(nn.Module):
    def __init__(self, d_enc_in: int, 
                cond_dim: int, 
                latent_dim: int, 
                width: float = 1.0, 
        ...
        assert d_hidden == d_enc_in
        self.pre = nn.Sequential(nn.LayerNorm(d_enc_in), nn.Linear(d_enc_in, d_hidden), nn.SiLU())
        self.film1 = LowRankFiLM(cond_dim, d_hidden, rank_film)
        self.res1 = nn.Sequential(nn.LayerNorm(d_hidden), LowRankResidual(d_hidden, rank_res), nn.SiLU(), nn.Dropout(dropout_p))
        self.film2 = LowRankFiLM(cond_dim, d_hidden)
        self.res2 = nn.Sequential(nn.LayerNorm(d_hidden), LowRankResidual(d_hidden, rank_res), nn.SiLU(), nn.Dropout(dropout_p))
...
class LoRACompressionModelV3(CompressionModel):
    def __init__(
        self,
        *,
        ...
        # FiLM 코어(funnel)
        self.encoder_core = EncoderCoreV3(d_enc_in=d_enc_in, cond_dim=cond_dim, latent_dim=latent_dim, width=width, rank_film=rank_film, rank_res=rank_res)
        self.decoder_core = DecoderCoreV3(latent_dim=latent_dim, cond_dim=cond_dim, d_dec_out=d_dec_out, width=width, rank_film=rank_film, rank_res=rank_res)
...
def get_compnet_v3(
    ...
    comp_model = LoRACompressionModelV3(
        ...
        rank_film= args.rank_film,
        rank_res= args.rank_res,
    ).to(device)
    return comp_model