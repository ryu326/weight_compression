...
def get_high_freq(x, radius_denominator = 8):
    ...
    lpf = torchvision.transforms.functional.pil_to_tensor(mask.convert('L'))[0].type(torch.float32) / 255.0 # 0 아니면 1로
...
def separate_high_not_high(x, num_of_special_patch_list = None):
    ...
    # 평균 구한걸 접음
    threshold_x_high_freq = threshold_x_high_freq.unfold(1, patch_size, patch_size).unfold(2, patch_size, patch_size).flatten(-2).mean(dim=-1)
    threshold_x_high_freq = threshold_x_high_freq.flatten(-2).contiguous()

    x_folding = rearrange(x, 'b c (h1 h2) (w1 w2) -> b c h1 w1 h2 w2', h1 = h//patch_size, w1 = w//patch_size, h2 = patch_size, w2 = patch_size).contiguous()
    ...
    for i in range(x_folding.size(0)):
        ...
        if num_of_special_patch_list == None:
            more_than_th = torch.nonzero(torch.where(topk_info.values > high_freq_decision_ratio_threshold, 1.0, 0.0)).size(0)
