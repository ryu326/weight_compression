    ...
    for dataset in dataset_folder_dict:
        ...
        for lmbda in [0.0007, 0.0025, 0.0035, 0.0067, 0.013]:
            ...
            file_list = os.listdir(
                f"./checkpoint/exp_NIC_Fair_lambda_{lmbda}_seed_100.0_batch_size_8_radius_denominator_8.0"
            )
            ...
            stat_csv = {"image_name": [], "bpp": [], "psnr": [], "lpips": []}

            mean_csv = {"bpp": 0.0, "psnr": 0.0, "lpips": 0.0}
            ...
            for image_name in tqdm(image_list, desc=f"Model lambda: {lmbda}, Dataset: {dataset}"):
                ...
                out_dec = net.decompress(
                    out_enc["strings_high_freq"],
                    out_enc["strings_not_high_freq"],
                    out_enc["shape"],
                    out_enc["topk_indices"],
                )

                num_pixels = x.size(0) * x.size(2) * x.size(3)

                bpp = (
                    sum(len(s[0]) for s in out_enc["strings_high_freq"]) * 8.0 / num_pixels
            ...
            print(
                f"Compression complete. {lmbda}, Dataset: {dataset}, BPP: {mean_csv['bpp']}, PSNR: {mean_csv['psnr']}, LPIPS: {mean_csv['lpips']}"
            )
