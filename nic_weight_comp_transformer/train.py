...
def main(opts):
    ...
    net = TR_NIC(input_dim= opts.data_dim) # 들어갈 변수 많으니 argument 받아서 잘 집어넣어주는 코드 짜보자 (hint: opt 이용)
    net = net.to(device)
    ...
    ########################################
    # test 준비
    test_dataset = WParam_dataset(dataset_folder= opts.dataset_dir, split='val', data_dim=opts.data_dim, length=opts.length, seed = 100)
    ...
    while(1):
        ...
        for i, (img) in enumerate(train_dataloader):
            ...
            wandb.log({"train_loss": out_criterion["loss"], "train_mse_loss": out_criterion, "train_bpp_loss": out_criterion["bpp_loss"]})
            optimizer.zero_grad()
            aux_optimizer.zero_grad()
            ...
            # if (total_iter % 50000 == 0):
                ...
                with torch.no_grad() :
                    ...
                    net_eval = TR_NIC(input_dim= opts.data_dim) # 들어갈 변수 많으니 argument 받아서 잘 집어넣어주는 코드 짜보자 (hint: opt 이용)
                    ...
                    if (node_rank == 0):
                        avg_bpp, mean_MSE = test(total_iter, test_dataset, net_eval, f"{save_path}/figures/total_iter_{total_iter}", logger, mse_func, node_rank)
                        wandb.log({"Test_avg_bpp": avg_bpp,  "Test_loss": mean_MSE})
                    else:
                        _ = test(total_iter, test_dataset, net_eval, f"{save_path}/figures/total_iter_{total_iter}", logger, mse_func, node_rank)
                ...
                if (node_rank == 0):
                    ...
                    if mean_MSE < best_mse :
                        ...
                        best_mse_model_path = save_path + '/' + f'best_mse_model_MSE_{round(mean_MSE, 5)}_BPP_{round(avg_bpp, 5)}_total_iter_{total_iter}.pth.tar'
                    ...
                    if avg_bpp < best_bpp : 
                        ...
                        best_bpp_model_path = save_path + '/' + f'best_bpp_model_MSE_{round(mean_MSE, 5)}_BPP_{round(avg_bpp, 5)}_total_iter_{total_iter}.pth.tar'
                        torch.save({
                            "state_dict": state_dict,
                            "optimizer": optimizer.state_dict(),
                            "aux_optimizer": aux_optimizer.state_dict(),
                    ...
                    recent_saved_model_path = save_path + '/' + f'recent_model_MSE_{round(mean_MSE, 5)}_BPP_{round(avg_bpp, 5)}_total_iter_{total_iter}.pth.tar'
...
def ddp_or_single_process(argvs):
    ...
    if opts.seed is not None:
        ...
        logger.info(f"exp name : exp_NIC_Fair_model_{opts.model_name}_lmbda_{opts.lmbda}_seed_{opts.seed}_batch_size_{opts.batch_size}_radius_denominator_{opts.radius_denominator}_total_iter_{opts.iter}")
