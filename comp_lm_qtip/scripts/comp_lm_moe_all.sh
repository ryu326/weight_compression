...
comp_model_bases=(
    ...
    "/home/jgryu/workspace/weight_compression/NWC/checkpoint2/nwc_ql/block_seq_ql_random_scaler_meta-llama--Meta-Llama-3-8B__col_1024_gaussian_padding.pt/M16_seed2_rdloss_ql_size16_encdim512_M16_Q4_R0_m0_batch_size2048_total_iter200000_lr0.0001_seed100"
    "/home/jgryu/workspace/weight_compression/NWC/checkpoint2/nwc_ql/block_seq_ql_random_scaler_meta-llama--Meta-Llama-3-8B__col_1024_gaussian_padding.pt/M16_seed3_rdloss_ql_size16_encdim512_M16_Q4_R0_m0_batch_size2048_total_iter200000_lr0.0001_seed3.0"
...
experiment_names=(
    "ql_ldlq64_rnorm_ft_seed4"
    "ql_ldlq64_rnorm_ft_seed2"
    "ql_ldlq64_rnorm_ft_seed3"
    # "ql_ldlq64_rnorm_ft"
    # "ql_ldlq64_rnorm"
)
##########################################################################
...
hess_paths=(
    # "/home/jgryu/workspace/weight_compression/Wparam_dataset/quip_hess/Qwen3-30B-A3B"
...
lmbda_values=(30 50 75 100 300 350 1000 1600 10000)
##########################################################################
##                        MAIN EXECUTION LOOP                           ##
##########################################################################
    ...
    for i in "${!experiment_names[@]}"; do
        ...
        for lmbda in "${lmbda_values[@]}"; do
            ...
            python -m eval.eval_zeroshot_hf \
                ...
                2>&1 | tee -a $LOG/${SAVE_NAME}_eval.log
            zeroshot_exit_code=${PIPESTATUS[0]}


            if [ "$ppl_exit_code" -eq 0 ] && [ "$zeroshot_exit_code" -eq 0 ]; then
                echo "All evals finished successfully. Cleaning up ckpt & hf..."
                if [ "$CKPT/$SAVE_NAME" != "$CKPT" ]; then
                    rm -rf "$CKPT/$SAVE_NAME"
                fi
            else
                echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
                echo "CRITICAL: Eval failed (ppl=$ppl_exit_code zeroshot=$zeroshot_exit_code)."
                echo "Skipping ckpt cleanup to allow manual inspection: $SAVE_NAME"
                echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
            fi
            rm -rf "$HF/$SAVE_NAME"
...
curl -s -X POST -H 'Content-type: application/json' \
    --data "{\"text\":\"[comp_lm_moe_all.sh] All experiments finished at $(date '+%Y-%m-%d %H:%M')\"}" \
    "https://hooks.slack.com/services/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" > /dev/null 2>&1
