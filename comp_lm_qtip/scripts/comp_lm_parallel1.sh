...
comp_model_bases=(
    '/home/jgryu/workspace/weight_compression/NWC/checkpoint2/nwc_ql/block_seq_ql_random_scaler_meta-llama--Meta-Llama-3-8B__col_1024_gaussian_padding.pt/rdloss_ql_size16_encdim512_M16_Q8_nRB4R0_m0_batch_size2048_total_iter200000_lr0.0001_seed100'
    '/home/jgryu/workspace/weight_compression/NWC/checkpoint2/nwc_ql/block_seq_ql_random_scaler_meta-llama--Meta-Llama-3-8B__col_1024_gaussian_padding.pt/rdloss_ql_size16_encdim512_M16_Q16_nRB4R0_m0_batch_size2048_total_iter200000_lr0.0001_seed100'
    # "../NWC/checkpoint/nwc_ql/block_seq_ql_random_scaler_meta-llama--Meta-Llama-3-8B__col_1024_gaussian_padding.pt/M16"
)
quantize_flags=(
    "--direction col --ql --Q 8 --row_normalize"
    "--direction col --ql --Q 16 --row_normalize"
    # "--direction col --ql --ql_random_uniform --Q 4 --row_normalize"
...
for j in "${!model_names[@]}"; do
    ...
    for i in "${!experiment_names[@]}"; do
        ...
        for lmbda in "${lmbda_values[@]}"; do
            ...
            echo "Sleeping ${JOB_LAUNCH_DELAY_SEC}s before launching the next job..."
            sleep "$JOB_LAUNCH_DELAY_SEC"
