#!/bin/bash
# Layer 5 ECFT: R_target=4.2, decoder=rht, lr=1e-3, lambda sweep
...
exp_group="ecft_layer5_rht_r42_sweep"
...
for idx in "${!ecft_lmbda_list[@]}"; do
    ...
    (
        echo "================================================================" | tee "$log_path"
        echo "[LAYER5 RHT R=4.2] lmbda=${ld} ft_lr=1e-3 GPU=${gpu_id}" | tee -a "$log_path"
        echo "================================================================" | tee -a "$log_path"
        ...
        CUDA_VISIBLE_DEVICES="${gpu_id}" python -m quantize_llama.quantize_finetune_llama \
            ...
            --ecft_lmbda "${ld}" \
            --ft_lr 1e-3 \
            --use_wandb --wandb_project "ecft_layer5_rht" --wandb_group "${exp_group}" \
            ${base_flags} \
            2>&1 | tee -a "$log_path"
    ) &
    pids+=($!)
