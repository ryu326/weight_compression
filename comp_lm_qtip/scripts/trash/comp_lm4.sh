...
for lmbda in "${lmbda_values[@]}"; do
    ...
    SAVE_NAME=${model_name}/ql_code_optim200_rnorm_round/lmbda${lmbda}
    pretrain_path=$HF/${SAVE_NAME}
    echo "################## Running hfize ${pretrain_path} ##################"

    python -m quantize_llama.hfize_llama --quantized_path $CKPT/${model_name}/ql_code_optim200_rnorm/lmbda${lmbda} \
            --W_key _round \
            --hf_output_path $pretrain_path \
