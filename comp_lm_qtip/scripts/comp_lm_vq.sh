...
# lmbda_values=(4 8 64 128)
# lmbda_values=("3.0bit_K64_e_dim2")
lmbda_values=(
    # "3.0bit_K64_e_dim2"
    # "2.0bit_K256_e_dim4"
    "2.5bit_K32_e_dim4"
...
for j in "${!model_names[@]}"; do
    ...
    for i in "${!experiment_names[@]}"; do
        ...
        for lmbda in "${lmbda_values[@]}"; do
            ...
            SAVE_NAME=${model_name}/${exp_name}/${lmbda}
            mkdir -p $(dirname "$LOG/$SAVE_NAME.log")

            echo "################## Running compression | lmbda=${lmbda} | Exp: ${exp_name} | Model: ${model_name} ##################"
            comp_model=$comp_model_base/${lmbda}*/best_loss*.pth.tar
            # comp_model=$comp_model_base/${lmbda}bit_*/best_loss*.pth.tar
            # taskset -c 0-63 \
