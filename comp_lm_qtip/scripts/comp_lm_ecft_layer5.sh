#!/bin/bash
# Layer 5 ECFT: R_target=3, lr=0.001, lambda sweep
...
exp_group="ecft_layer5_r4.2_rht_full"
...
for idx in "${!ecft_lmbda_list[@]}"; do
    ...
    (
        echo "================================================================" | tee "$log_path"
        echo "[LAYER5 R=3] lmbda=${ld} ft_lr=1e-3 GPU=${gpu_id}" | tee -a "$log_path"
        echo "================================================================" | tee -a "$log_path"
        ...
        CUDA_VISIBLE_DEVICES="${gpu_id}" python -m quantize_llama.quantize_finetune_llama \
            ...
            --ecft_lmbda "${ld}" \
            --ft_lr 1e-3 \
            --use_wandb --wandb_project "ecft_layer5_r3" --wandb_group "${exp_group}" \
            ${base_flags} \
            2>&1 | tee -a "$log_path"
    ) &
    pids+=($!)
