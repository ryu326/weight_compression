...
def code_optimize(W, HR, comp_model, args, **kwargs):
    ...
    for start_idx in range(0, W.shape[0], bs):
        ...
        bpp_sum += bpp
    print(f"num_pixels: {num_pixels}, bpp_loss_sum: {bpp_loss_sum}, bpp_sum: {bpp_sum}")
    ...
    optimizer, aux_optimizer, code_optimizer = configure_optimizers(comp_model, args, [code_latent])
        ...
        for epoch in range(args.code_optim_it):
            ...
            for batch in loader:
                ...
                if qlevel is None:
                    ...
                    w_batch, ql_batch = batch
                code_batch = code_latent[start_idx:end_idx]
                code_batch.retain_grad()
                # print(code_batch.grad is not None)
                ...
                # print(code_batch.grad)
                # print(code_batch.requires_grad)
                # print(code_latent.grad)
                # print(code_latent.requires_grad)
                # print(f"Mean grad: {code_latent.grad.mean().item()}")
                
                code_optimizer.step()

                code_latent[start_idx:end_idx] = code_batch.detach().clone()
                ...
                recon_loss_total += loss['recon_loss'].item() * batch_size
            ...
            recon_loss_per_epoch = recon_loss_total / num_pixels
            ...
            ft_result['recon_loss_per_epoch'].append(recon_loss_per_epoch)
