...
def compress_linear(W, H, comp_model, Qlevel, args, device='cpu'):
    ...
    if args.ldlq:
        ...
        import lib.algo.code_optimize as fine_tune_comp_model_v3
        out, ft_result = fine_tune_comp_model_v3(W, H, comp_model, args, qlevel = Qlevel)
    elif args.code_optim:
        from lib.algo.code_optimize import code_optimize
        out, ft_result = code_optimize(W, H, comp_model, args, qlevel = Qlevel)
    ...
    if args.row_normalize and args.col_normalize:
        out['W_hat'] = out['W_hat'] * col_std  # col 먼저 복원
        out['W_hat'] = out['W_hat'] * row_std  # 그 다음 row 복원
    elif args.row_normalize:
        out['W_hat'] = out['W_hat'] * row_std
    elif args.col_normalize:
        out['W_hat'] = out['W_hat'] * col_std
...
def pseudo_compress_tensor(W, model, args, **kwargs):
    ...
    for start_idx in range(0, W.shape[0], bs):
        ...
        x_hat, n_pixels, bpp_loss_, out, out_enc, nbits = model_foward(batch, model, args, ql = ql)
...
def pseudo_compress_tensor_ldlq(Wr, H, model, args, **kwargs):
    ...
    for k in reversed(range(n)):
        ...
        x_hat, n_pixels, bpp_loss_, out, out_enc, nbits = model_foward(WXWX.reshape(1, -1), model, args, ql = ql)
    ...
    for ie in range(args.quip_tune_iters):
        ...
        for k in reversed(range(n)):
            ...
            x_hat, n_pixels, bpp_loss_ = model_foward(WXWX.reshape(1, -1, model.input_size), model, ql)
...
def pseudo_compress_tensor_ldlq_block(Wr, H, model, args, **kwargs):
    assert args.direction == 'col'
    ...
    qlevel = kwargs.get('qlevel', None)
    # for k in reversed(range(n)):
    for k in reversed(range(0, n, bs)):
        WXWX = Wr[:, k:k+bs] + (Wr[:, k+bs:] - hatWr[:, k+bs:]) @ L[k+bs:, k:k+bs]
    
        ql = None if qlevel is None else qlevel[k]
                
        x_hat, n_pixels, bpp_loss_, out, out_enc, nbits = model_foward(WXWX.T, model, args, ql = ql)
        
        # q = x_hat.flatten()
        # hatWr[:, k:k+bs] = q
        hatWr[:, k:k+bs] = x_hat.T
        num_pixels += n_pixels
        bpp_loss_sum += bpp_loss_
        codes.append(out_enc)
        bpp_sum += nbits
