...
def compress_linear(W, H, comp_model, Qlevel, args, device='cpu'):
    ...
    res = comp_W(W, H, comp_model, args, qlevel = Qlevel, row_norm=row_std, col_norm=col_std, qmap = Qmap, scaleH = scaleH)
...
def comp_W(W, H, model, args, **kwargs):
    ...
    scaleH = kwargs.get('scaleH', None)  # (1, n)
    ...
    for i,e in enumerate(range(n, 0, -bs)):
        ...
        sc = scaleH[None, :][:, s:e] if args.scale_cond0 else None
        ...
        x_hat, n_pixels, bpp_loss_, out, out_enc, nbits = model_foward_one_batch(w.clone(), model, args, ql = ql, qm = qm, rnorm = r_norm, cnorm = c_norm, sc = sc)
...
def model_foward_one_batch(w, model, args, **kwargs):
    ...
    scale_cond = sc.repeat(m, 1) if args.scale_cond0 else None
