...
def _fmt_value(x, digits: int = 6) -> str:
    x = _to_scalar(x)
    if x is None:
        return "-"
...
def _decoder_totals_from_out(out):
    if not isinstance(out, dict):
        return None, None
    decoder_bits = out.get("decoder_bits", {})
    num_params = _to_scalar(out.get("num_pixels"))
    decoder_total_raw = None
    if isinstance(decoder_bits, dict):
        decoder_total_raw = _to_scalar(decoder_bits.get("total"))
    decoder_total = None
    if (
...
def compress_finetune_decoder_layer(mixed_layer, quant_order, idx, comp_model, ql_i, args,
    ...
    for quant_i, (linear_attr, name, in_hess_name, out_hess_name,
        ...
        num_pixels = float(_to_scalar(out["num_pixels"]))
        metadata_total = metadata_total_raw / num_pixels
        bpp_sum = float(_to_scalar(out["bpp_sum"]))
        bpp_loss_sum = float(_to_scalar(out["bpp_loss_sum"]))
        bpp_with_metadata = (bpp_sum + metadata_total_raw) / num_pixels
        bpp_loss_with_metadata = (bpp_loss_sum + metadata_total_raw) / num_pixels

        decoder_total, decoder_total_raw = _decoder_totals_from_out(out)
        torch.save(
            {
    ...
    if args.ft_epochs > 0:
        if args.ft_rnorm:
            ...
            for quant_i, (linear_attr, name, in_hess_name, out_hess_name,
                ...
                w_shape = tuple(data['hatWr'].shape)
                metadata_total_raw = float(utils.calculate_metadata_bpp(data['metadata'], w_shape, args))
                num_pixels = float(_to_scalar(data['num_pixels']))
                bpp_sum = float(_to_scalar(data['bpp_sum']))
                bpp_loss_sum = float(_to_scalar(data['bpp_loss_sum']))
                data['metadata_total_raw'] = metadata_total_raw
                data['metadata_total'] = metadata_total_raw / num_pixels
                data['bpp_with_metadata'] = (bpp_sum + metadata_total_raw) / num_pixels
                data['bpp_loss_with_metadata'] = (bpp_loss_sum + metadata_total_raw) / num_pixels
                torch.save(data, save_path)
        # elif args.ft_y:
        #     for quant_i, (linear_attr, name, in_hess_name, out_hess_name,
        #                 rcp) in enumerate(quant_order):
