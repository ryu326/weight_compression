...
def model_from_hf_path_gptoss(path, max_mem_ratio=0.7, device_map=None, sep_rnorm=False, gptoss_replace_version=None):
    if ('gptoss' in path or 'gpt-oss' in path) and gptoss_replace_version == 'standard':
...
def replace_gptoss_modules_v1(model, v11 = False):
    glog.info("🔄 Standard Module V1 교체 작업 시작...")
    config = model.config
    ...
    for i, layer in enumerate(model.model.layers):
        ...
        if i == 0:
            glog.info(f"   ℹ️ Layer 0 변환 완료 (Alpha: {orig_alpha}, Limit: {orig_limit})")

    glog.info("✅ 모든 레이어의 V1 모듈 교체가 완료되었습니다!")
    return model
