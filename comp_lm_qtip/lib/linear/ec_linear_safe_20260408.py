...
class EntropyConstrainedLinear(Module):
    ...
    def __init__(
        ...
        entropy_chunk_rows: Optional[int] = None,
        device=None,
        dtype=None,
    ) -> None:
        ...
        self.entropy_chunk_rows = (
            int(entropy_chunk_rows) if entropy_chunk_rows is not None else 0
        )
        ...
        self._last_likelihoods: Optional[Tensor] = None
    ...
    def quantize_latent(
        ...
        if chunk_rows > 0 and latent.ndim == 4 and latent.shape[2] > chunk_rows:
            quantized = torch.empty_like(latent)
            likelihoods_full = torch.empty_like(latent) if return_likelihoods else None
            ...
            for start in range(0, latent.shape[2], chunk_rows):
                ...
                q_chunk, l_chunk = self.entropy_bottleneck(
                    latent[:, :, start:end, :], training=training
                )
                quantized[:, :, start:end, :] = q_chunk
                if likelihoods_full is not None:
                    likelihoods_full[:, :, start:end, :] = l_chunk
            ...
            likelihoods = likelihoods_full
        else:
            quantized, likelihoods_raw = self.entropy_bottleneck(latent, training=training)
            self._last_rate_loss = (
                -torch.log2(likelihoods_raw.float().clamp_min(1e-9))
            ).mean()
            likelihoods = likelihoods_raw if return_likelihoods else None
    ...
    def forward(
        ...
        self._last_likelihoods = likelihoods if return_likelihoods else None
    ...
    def extra_repr(self) -> str:
        return (
            ...
            f"entropy_chunk_rows={self.entropy_chunk_rows}"
        )
