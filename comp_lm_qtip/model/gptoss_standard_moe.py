...
class GptOssExperts(nn.Module):
    def __init__(self, config):
        ...
        self.experts = nn.ModuleList([GptOssLayer(config) for _ in range(self.num_experts)])
    ...
    def forward(self, hidden_states: torch.Tensor, router_indices=None, routing_weights=None) -> torch.Tensor:
        ...
        next_states = torch.zeros_like(hidden_states, dtype=hidden_states.dtype, device=hidden_states.device) # (num_tokens, hidden_size) 
        
        with torch.no_grad():
            expert_mask = torch.nn.functional.one_hot(router_indices, num_classes=num_experts)  # (num_tokens, top_k, num_experts) 
            expert_mask = expert_mask.permute(2, 1, 0) # (num_experts, top_k, num_tokens)
            expert_hitted = torch.greater(expert_mask.sum(dim=(-1, -2)), 0).nonzero() # (hitted_expert, 1)
            print('expert_hitted:',  expert_hitted.shape)

        for expert_idx in expert_hitted[:]:  
            idx_int = expert_idx.item() # (1,) --> integer
...
class GptOssLayer(nn.Module):
    """
    단일 Expert 모듈입니다. (Mixtral의 BlockSparseTop2MLP에 대응)
    양자화 툴은 이 클래스 내부의 Linear 레이어(gate_up_proj, down_proj)를 찾아 양자화합니다.
    """
    def __init__(self, config):
        super().__init__()
