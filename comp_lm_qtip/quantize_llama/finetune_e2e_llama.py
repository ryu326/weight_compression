...
def main(args):
    ...
    quant_model = model_from_hf_path(args.hf_path,
                                     device_map=fake_dev_map, sep_rnorm = args.sep_rnorm)[0].float()

    for name, module in quant_model.named_modules():
        if isinstance(module, nn.Linear):
            module.weight.requires_grad = False
        elif isinstance(module, CompLinear):
            module.Wr.requires_grad = False
            module.row_norm.requires_grad = True
            
    utils.clean()
    with torch.enable_grad():
        finetune.finetune_e2e(quant_model, start_dev, devset, orig_dtype,
                                   args)
