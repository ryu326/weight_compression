...
def main(args):
    ...
    for ii in range(len(model.model.layers)):
        ...
        for name, submodule_name, proj_name in weight_types:
            ...
            W = getattr(orig_submodule, proj_name).weight.data
            
            W = W.cuda()
            W_hat = W_hat.cuda()
            SE = (W_hat - W)**2 / W.std()
            mse_col = SE.mean(dim = 0)
            
            # comp_result[f'{skip_key}.pt']['mse_col'] = mse_col.tolist()
            comp_result[f'{skip_key}.pt']['mse_col_min'] = mse_col.min().item()
            comp_result[f'{skip_key}.pt']['mse_col_max'] = mse_col.max().item()
            comp_result[f'{skip_key}.pt']['mse_col_mean'] = mse_col.mean().item()     
            comp_result[f'{skip_key}.pt']['mse_col_std'] = mse_col.std().item()           
            comp_result[f'{skip_key}.pt']['mse_col_range'] = mse_col.max().item() - mse_col.min().item()             
            
        glog.info(f'loaded layer {ii}')
        del W_hat, W, SE, mse_col
    ...
    file_path = f'{args.hf_output_path.replace(HF, RES)}_mse_col.json'
