...
for model_name in "${model_names[@]}"; do
    ...
    for lmbda in "${lmbda_values[@]}"; do
        ...
        python -m quantize_llama.quantize_finetune_llama --save_path $CKPT/$model_name/lmbda${lmbda}_had \
            ...
            --incoh_mode had \
            --ft_epochs 0 \
            --comp_model_path $comp_model 2>&1 | tee $LOG/${model_name}_lmbda${lmbda}_had.log
        
        python -m quantize_llama.hfize_llama --quantized_path $CKPT/$model_name/lmbda${lmbda}_had \
                --hf_output_path $HF/$model_name/lmbda${lmbda}_had 2>&1 | tee $LOG/${model_name}_lmbda${lmbda}_had_hfize.log 

        pretrain_path=$HF/$model_name/lmbda${lmbda}_had
