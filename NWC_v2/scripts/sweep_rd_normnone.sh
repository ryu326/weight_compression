#!/bin/bash
# RD-curve sweep with no normalization (llama8b uses raw patches).
#   4 configurations:
#     A: encoder=resblock(n=2), decoder=linear
#     B: encoder=rht,           decoder=rht          (2D RHT, 128x128)
#     C: encoder=linear,        decoder=linear
#     D: encoder=resblock(n=2), decoder=resblock(n=2)
#   datasets   = {gaussian, llama8b}    (llama8b: --normalize none)
#   lambdas    = {32, 128, 1024, 8192}
#   entropy    = compressai (fixed)
#   early stop = patience=3 on val/loss, min_iter=5000
# Total: 4 × 2 × 4 = 32 runs.
#
# Output: ./checkpoint/rd_sweep_normnone/<run_name>/best.pth.tar
    ...
    for dataset in "${datasets[@]}"; do
        for lmbda in "${lambdas[@]}"; do
            ...
            if [[ "$dataset" == "llama8b" ]]; then
                # No normalization: pass --normalize none and chosen direction.
