...
def main(opts):
    ...
    criterion = get_loss_fn(opts, std=train_std, coff= torch.tensor([3.4, 1.2, 0.1, 0.05]).to(device))
...
def ddp_or_single_process(argvs):
    ...
    folder_name += f"/lmbda{opts.lmbda}_{opts.loss}_encdim{opts.dim_encoder}_M{opts.M}"
