...
def reconstruct_model(state_dict, model, input_mag, direction, batch_size=32768 // 8):
    ...
    with torch.no_grad():
        ...
        for k, W in tqdm(state_dict.items()):
            ...
            input_block = input_block.expand(rows // model.input_size, cols)
...
if __name__ == "__main__":
    ...
    save_directory = (
        f"/home/jgryu/Weight_compression/model_reconstructed/nwc_ql_test/{(args.direction).upper()}_{os.path.join(*args.model_path.split('/')[-3:])}_MSE_{round(mse_total, 5)}_bpploss{round(bpp_loss, 5)}"
    )
