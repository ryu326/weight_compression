...
class ColBlock_qlevel(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # input_block = torch.sqrt(input_block)
        
        ## v2
        q_level = input_block[col_idx]

        # ## v1
        # quality = torch.zeros_like(input_block, dtype=torch.int32)
        # _, topk_indices = torch.topk(input_block, k=114)
        # # import ipdb; ipdb.set_trace()
        # start = 0
        # for count, value in zip([4, 10, 100] , [3, 2, 1]):
        #     indices = topk_indices[start:start + count]
        #     quality[indices] = value
        #     start += count
        # q_level = quality[col_idx]
        # print(q_level.shape, quality.shape)
...
def get_dataset_block_qlevel(direction, size):
    ...
    ## v2
    layer_inputs = torch.load('../Wparam_dataset/calib_data/layer_inputs_chmag_rank_top[4, 10, 100]_qlevel[3, 2, 1].pt')
    
    if direction == 'col':
        tensor_block_idx_train = torch.load(f'../Wparam_dataset/block_{size}_idx/tensor_block_col_idx_train.pt')
        tensor_block_idx_val = torch.load(f'../Wparam_dataset/block_{size}_idx/tensor_block_col_idx_val.pt')       
        
        with open(f'../Wparam_dataset/dataset_block/meta-llama/Meta-Llama-3-8B/mlp_attn_{size}_col_dataset_stats.json', "r", encoding="utf-8") as file:
            dataset_stats = json.load(file)  # JSON 파일을 Python 객체로 변환

        train_dataset = ColBlock_qlevel(net, tensor_block_idx_train, layer_inputs, dataset_stats["train"])
        valid_dataset = ColBlock_qlevel(net, tensor_block_idx_val, layer_inputs, dataset_stats["val"])
