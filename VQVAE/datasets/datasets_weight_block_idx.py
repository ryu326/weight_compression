...
def get_dataset_block_idx(direction, size):
    ...
    if direction == 'col':
        ...
        tensor_block_idx_train = torch.loadf('../Wparam_dataset/block_{size}_idx/tensor_block_row_idx_train.pt')
        tensor_block_idx_val = torch.load(f'../Wparam_dataset/block_{size}_idx/tensor_block_row_idx_val.pt')       
        
        with open(f'../Wparam_dataset/dataset_block/meta-llama/Meta-Llama-3-8B/mlp_attn_{size}_row_dataset_stats.json', "r", encoding="utf-8") as file:
            dataset_stats = json.load(file)  # JSON 파일을 Python 객체로 변환
