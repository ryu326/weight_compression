    ...
    def __getitem__(self, idx):
        ...
        try: 
            ...
            if weight.numel() < 4096:
                padding = 4096 - weight.numel()
                weight = torch.cat([weight, torch.zeros(padding, dtype=weight.dtype)])
            weight = weight.view(256, 16)
            
            
            weight = weight.view(256, 16)
        
        scale = torch.randint(0, 256, (1,)) + 1
...
def get_dataset_vector_random_scale(direction = 'row'):
    ...
    if direction == 'col':
        pass
        tensor_block_idx_train = torch.load('../Wparam_dataset/vector_idx/col_idx_train.pt')
        tensor_block_idx_val = torch.load('../Wparam_dataset/vector_idx/col_idx_val.pt')       
        ...
        train_dataset = ColVec_Scale(net, tensor_block_idx_train, layer_inputs, dataset_stats["train"])
        valid_dataset = ColVec_Scale(net, tensor_block_idx_val, layer_inputs, dataset_stats["val"])
