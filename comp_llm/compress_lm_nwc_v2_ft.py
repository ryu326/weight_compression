...
## 한 block ft할 때, 한 layer 압축하고 다른 레이어 ft 할 때마다 실행되는 함수수
def finetune_decoder_layer(layer, name, device, train_dl, valid_dl, args):
    layer = layer.to(device)

    params = utils.extract_ft_params(layer)
    optim = utils.get_adam(params, args)
...
## ft할 때 block마다 실행되는 함수
def compress_finetune_decoder_block(block, comp_order, idx, pre_orig_emb, orig_emb, device, args):
    ...
    for comp_layer_name in comp_order:
        W = layers[comp_layer_name].weight.data.detach()
        ...
        layers[comp_layer_name].weight.data = W_hat
        
        for param in layers[comp_layer_name].parameters():  # self-attention 부분만 고정
            param.requires_grad = False
        
        finetune_decoder_layer(block, comp_layer_name, device, train_dl, valid_dl, args)

        assert torch.equal(W_hat, dict(block.named_modules())[comp_layer_name])
