...
def pseudo_compress_tensor_gptq(W, model, direction, bs, ch_rank, H):
...
def pseudo_compress_tensor_ldlq(Wr, model, direction, bs, ch_rank, H):
    assert direction == 'col'
    ...
    for k in reversed(range(n)):
        ...
        data = {}
        if ch_rank is not None:
            assert ch_rank.shape[0] == n
            data['q_level'] = ch_rank[k].unsqueeze(0).to(device)
        data['weight_block'] = WXWX.reshape(1, -1, model.input_size).cuda()
        out = model(data)
        q = out["x_hat"].flatten()
        # import ipdb; ipdb.set_trace()
        hatWr[:, k] = q
        
        num_pixels += WXWX.numel()
        if isinstance(out["likelihoods"], dict):
            bpp_loss += sum(
                (torch.log(likelihoods).sum() / -math.log(2))
                for likelihoods in out["likelihoods"].values()
