...
def control_weights(args, models):
    ...
    def init_weights(m):
        if hasattr(m, "weight") and hasattr(m.weight, "uniform_") and args.init > 0.0:
...
def package(X, Y):
    return torch.cat([X.repeat_interleave(X.size(0), 0), Y.repeat(Y.size(0), 1)], dim=1)
...
def main(args):
    ...
    ds = {"gaussian": synthetic.GaussianXZN, "uniform": synthetic.UniformsXZN, "tri": synthetic.MultiTriangleXZN}
    ...
    models = {
        "dv": util.SingleSampleEstimator(args, args.dim, args.hidden, args.layers, "dv"),
        "mine": util.MINE(args, args.dim, args.hidden, args.layers, carry_rate=args.carry),
        "nwj": util.SingleSampleEstimator(args, args.dim, args.hidden, args.layers, "nwj"),
        "nwjjs": util.NWJJS(args, args.dim, args.hidden, args.layers),
        "cpc": util.CPC(args, args.dim, args.hidden, args.layers),
        "interpol": util.Interpolated(args, args.dim, args.hidden, args.layers, args.a, args.alpha),
    ...
    optims = {name: torch.optim.Adam(models[name].parameters(), lr=args.lr) for name in models if name != "knife"}

    optims["knife"] = MultiOptimizer(
        torch.optim.Adam(models["knife"].kernel_marg.parameters(), lr=1e3 * args.lr),
    ...
    try:
        for step in range(1, args.steps + 1):
            if I_delta is not None and step % rho_iterations == 0:
                ...
                pXY.rho.data = torch.tensor([args.rho], requires_grad=True, dtype=torch.float, device=device)
...
def meta_main(args):
    ...
    for run_number in range(1, args.nruns + 1):
        ...
        for name in test_MI:
            if run_number == 1:
                ...
                if math.isnan(best_test_MI[name]) or abs(mi - test_MI[name]) < abs(mi - best_test_MI[name]):
                    logger.info(
                        "*** New best test MI %g for %s from previous best %g"
    ...
    for name in best_test_MI:
        logger.info("{:10s}: {:6.2f} \t\t {:s}".format(name, best_test_MI[name], str(bestargs[name])))
    logger.info("-" * 150)
...
def get_args(argv=None):
    ...
    parser.add_argument(
        "--c", type=int, default=1, help="c*N is the number of samples for final " "evaluation [%(default)g]"
    )
    ...
    parser.add_argument("--init", type=float, default=0.0, help="param init (default if 0) [%(default)g]")
    parser.add_argument("--clip", type=float, default=1, help="gradient clipping [%(default)g]")
    parser.add_argument("--steps", type=int, default=1000, metavar="T", help="number of training steps [%(default)d]")
    parser.add_argument("--carry", type=float, default=0.99, help="EMA carry rate [%(default)g]")
    parser.add_argument("--alpha", type=float, default=1.0, help="interpolation weight (on CPC term) [%(default)g]")
    parser.add_argument(
        "--a", type=str, default="e", choices=["e", "ff", "lp"], help="score function in TUBA, INTERPOL [%(default)s]"
    )
    parser.add_argument(
        "--nruns", type=int, default=1, help="number of random runs (not random if set to 1) " "[%(default)d]"
    )
    parser.add_argument("--pickle", type=str, default="cukes.p", help="output pickle file path [%(default)s]")
    parser.add_argument("--seed", type=int, default=None, help="random seed [%(default)s]")
    parser.add_argument("--cuda", action="store_true", help="use CUDA?")

    # Custom Arguments
    parser.add_argument("--debug", action="store_true", help="Debug loglevel")
    parser.add_argument("--logdir", type=str, default="./results", help="Root logging directory")
    parser.add_argument(
        "--run_id", type=str, default=None, help="Run ID for tensorboard logging; defaults to timestamp"
    )
    parser.add_argument(
        "--pdf", type=str, default="gaussian", choices=["gaussian", "uniform", "tri"], help="PDF to use"
    )
    parser.add_argument("--cubic", action="store_true", help="Apply cubic transformation?")
    parser.add_argument("--I", type=float, default=None, help="MI value to start")
    parser.add_argument("--I_max", type=float, default=None, help="MI value to end")
    parser.add_argument("--rho_steps", type=int, default=5, help="Number of steps for stepping up MI")

    # Arguments for KNIFE
    parser.add_argument("--tukey_transform", action="store_true", default=False, help="tukey_transform")
    parser.add_argument(
        "--no-optimize_mu", dest="optimize_mu", action="store_false", default=True, help="Optimize means of kernel"
    )
    parser.add_argument(
        "--cond_modes", type=int, default=128, help="Number of kernel components for conditional estimation"
    )
    parser.add_argument("--use_tanh", action="store_true", default=False, help="use tanh()")
    parser.add_argument("--init_std", type=float, default=0.01, help="std for initialization")
    parser.add_argument(
        "--cov_diagonal", type=str, default="var", help="Diagonal elements of cov matrix different or the same?"
    )
    parser.add_argument("--cov_off_diagonal", type=str, default="var", help="Off-diagonal elements of cov matrix zero?")
    parser.add_argument("--average", type=str, default="var", help="Weighted average?")
    parser.add_argument("--marg_modes", type=int, default=128, help="Kernel components for marginal estimation")
    parser.add_argument("--ff_residual_connection", action="store_true", default=False, help="FF Residual Connections")
    parser.add_argument("--ff_activation", type=str, default="tanh", help="FF Activation Function")
    parser.add_argument("--ff_layer_norm", default=False, action="store_true", help="Use a NormLayer?")
