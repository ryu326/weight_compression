...
def plot_entropy(filename, show=False, save=True):
    ...
    plt.step(list(data_to_plot["x"]), data_to_plot["loss_true_entropy"], label="True", color="black", linewidth=6)
    ...
    for k, v in data_to_plot.items():
        if k.startswith("y_"):
            ...
            if name == "Schraudolph":
                ...
                continue
            plt.plot(list(data_to_plot[x]), v, "-", label=lbl, color=color_palette[idx], alpha=0.7, linewidth=10)
            idx += 1
    plt.plot(list(data_to_plot["x_true"]), data_to_plot["y_true"], label="True", color="black", linewidth=1)
...
def plot_mi_bias_var_mse(filename, pdf_out=None, save=True, show=False):
    ...
    for i, model_name in enumerate(model_list):
        plt.sca(axs[0])
        plt.plot(
            MIs, biases[model_name], label=display_names[model_name], marker="d", color=colors[col_idx[i]]["color"]
        )
