...
def get_dataloader(data_generator, n_batchsize, n_iterations, cubic=False):
    ...
    dataloader = data.DataLoader(dataset, batch_size=n_batchsize, shuffle=True, num_workers=min(1, os.cpu_count() - 1))
    return dataloader
...
def main(args):
    ...
    if args.pdf == "gaussian":
        ...
        data_generator1 = get_random_data_generator(
            base_pdf=args.pdf,
            number=args.data_components,
        )
        data_generator = DataGeneratorMulti(data_generator1, d=d)

    dataloader = get_dataloader(data_generator, n_batchsize=n_batchsize, n_iterations=n_iterations, cubic=args.cubic)

    samples = data_generator.rvs(size=(n_samples_kernel,))
    if d == 1 and len(samples.shape) < 2:
        samples = samples[:, None]
    args_adaptive = SimpleNamespace(
        optimize_mu=True,
    ...
    args_schraudolph = SimpleNamespace(
        optimize_mu=False,
        marg_modes=args.kernel_samples,
        batch_size=args.kernel_samples,
        use_tanh=args.use_tanh,
    ...
    G_doe = Doe(args.dimension, "gauss")
            ...
            for i in tqdm(range(len(dataloader))):
                ...
                if (i + 1) % iter_delta == 0 and args.pdf == "gaussian":
                    ...
                    dataloader = get_dataloader(
                        data_generator, n_batchsize=n_batchsize, n_iterations=n_iterations, cubic=args.cubic
                    )
                    data_iter = iter(dataloader)
                    if args.cubic:
                        if args.pdf == "gaussian":
                ...
                if args.cubic:
                    y_oracle = (
                        args.dimension * np.log(3)
                        + 2 / 3 * np.sum(np.log(np.abs(x_np)), axis=-1)
                        - data_generator.logpdf(np.abs(x_np) ** (1 / 3))
                    )
                else:
...
def get_args(argv=None):
    ...
    parser.add_argument("--logdir", type=str, required=False, default="runs", help="Root logging dir")
    parser.add_argument("--run_id", type=str, default=None, required=False, help="tensorboard run id")
    parser.add_argument("--batchsize", type=int, required=False, default=128, help="Size of batches")
    parser.add_argument("--iterations", type=int, required=False, default=1000, help="Number of iterations per epoch")
    parser.add_argument("--epochs", type=int, required=False, default=1, help="Number of epochs")
    parser.add_argument(
        "--kernel_samples", type=int, required=False, default=500, help="Number samples used for kernel construction"
    )
    parser.add_argument("--rho", type=float, default=1.0, help="multiplicative constant for stepping")
    parser.add_argument("--lr", type=float, default=1e-3, help="learning rate")
    parser.add_argument("--rho_steps", type=int, default=1, help="how many steps")
    parser.add_argument("--cubic", action="store_true", default=False, help="Cubed transform")
    parser.add_argument("--use_tanh", action="store_true", default=False, help="Use Tanh for the variance")

    parser.add_argument("--plot", required=False, action="store_true", help="Plot the PDF")
    parser.add_argument(
        "--pdf",
        type=str,
        choices=(
    ...
    parser.add_argument("--dimension", type=int, required=False, default=5, help="Data dimension")
    parser.add_argument("--data_components", type=int, required=False, default=15, help="Number modes for the real pdf")
