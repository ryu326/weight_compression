...
class STL10(VisionDataset):
    ...
    def __init__(
        ...
        if self.split == "train":
            self.data, self.targets = self.__loadfile(self.train_list[0][0], self.train_list[1][0])
            self.data = self.data[:-n_valid]
            self.targets = self.targets[:-n_valid]
        ...
        if self.split == "valid":
            ...
            self.targets = np.concatenate((self.targets, np.asarray([-1] * unlabeled_data.shape[0])))

        elif self.split == "unlabeled":
            self.data, _ = self.__loadfile(self.train_list[2][0])
            self.targets = np.asarray([-1] * self.data.shape[0])

        elif self.split == "test":
            self.data, self.targets = self.__loadfile(self.test_list[0][0], self.test_list[1][0])
        ...
        self.targets = self.targets[targets != 7]  # remove monkey
        self.data = self.data[targets != 7]
        self.targets[self.targets > 7] -= 1  # translate all classes above monkey
    ...
    def __load_folds(self, folds: Optional[int]) -> None:
        ...
        with open(path_to_folds, "r") as f:
            ...
            self.data, self.targets = self.data[list_idx, :, :, :], self.targets[list_idx]
