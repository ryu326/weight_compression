...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--simu_params", nargs="+", type=str, default=["source_data", "target_data", "method"])
    parser.add_argument("--group_by", type=str, default="method")
    parser.add_argument("--out_type", type=str, default="latex")
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.out_type == "latex":
        ...
        for method in ndf.index.to_list():
            for source, target, mean, std in zip(
                ndf.loc[method, "source_data"],
                ndf.loc[method, "target_data"],
                ndf.loc[method, "mean"],
                ndf.loc[method, "std"],
            ):
                ndf.loc[method, f"{source} -> {target}"] = f"{mean} (\pm {std})"
            ndf.loc[method, "Mean"] = "{} (\pm {})".format(
                np.round(np.mean(ndf.loc[method, "mean"]), 2), np.round(np.mean(ndf.loc[method, "std"]), 2)
            )
