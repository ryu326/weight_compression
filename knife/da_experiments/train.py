...
def copy_config(args, exp_root):
    ...
    filtered_list = [
        ...
        if ("checkpoints" not in str(file) and "archive" not in str(file) and "results" not in str(file))
    ]
...
def get_loaders(args, data):
    ...
    val_loader = DataLoader(
        val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, drop_last=True
    )
...
def main_worker(rank: int, world_size: int, args: argparse.Namespace) -> None:
    ...
    if main_process(args):
        metrics: Dict[str, torch.Tensor] = {
            "train_loss_c": torch.zeros(int(args.num_updates / args.print_freq)),
    ...
    for i in tqdm_bar:
        ...
        y_d = torch.cat([torch.zeros_like(y_s), torch.ones_like(y_t)])
        ...
        if i % args.eval_freq == 0:
            ...
            print(
                "Iteration: [{}/{}] \t Validation_t: {:.3f} Test_t: {:.3f}".format(
                    i, args.num_updates, val_acc, test_acc
                )
            )  # noqa: E126
            if main_process(args):
                save_checkpoint(
...
def update_csv(simu_params: dict, metrics: dict, iteration: int, path: str):
    try:
