...
class StyleEmdedding(torch.nn.Module):
    ...
    def __init__(self, args, reny_dataloader):
        ...
        if self.args.complex_proj_content:
            self.proj_style = nn.Sequential(
                nn.Linear(1, args.style_dim),
                nn.LeakyReLU(),
                nn.Linear(args.style_dim, args.style_dim),
                nn.LeakyReLU(),  # emb labels
                nn.Linear(args.style_dim, args.style_dim),
            )  # emb labels
            ...
            self.proj_style = nn.Sequential(
                nn.Linear(1, args.style_dim), nn.LeakyReLU(), nn.Linear(args.style_dim, args.style_dim)
            )  # emb labels
            self.proj_content = nn.Sequential(
                nn.Linear(args.hidden_dim, args.hidden_dim),
                nn.LeakyReLU(),
    ...
    def forward(self, input_tensor, labels, teacher_ratio):
        ...
        reny_pos_class, reny_neg_class, reny_one_labels = (
            torch.tensor(0.0).to(self.args.device),
            torch.tensor(0.0).to(self.args.device),
            torch.tensor(0.0).to(self.args.device),
        )
        pos_class, neg_class, one_labels = (
            torch.tensor(0.0).to(self.args.device),
            torch.tensor(0.0).to(self.args.device),
            torch.tensor(0.0).to(self.args.device),
        )
                ...
                if not self.args.no_minimization_of_mi_training:
                    ...
                    if self.args.use_complex_gamma_training:
                        ...
                        label_u = torch.tensor(
                            [[1.0, 0.0] if el == 0 else [0.0, 1.0] for el in labels_reny.tolist()]
                        ).to(self.args.device)
                        ...
                        loss_gamma = (
                            -torch.mean(d_gamma_content_pred_u[:, 0]) / 2 - torch.mean(d_gamma_content_pred_v[:, 1]) / 2
                        )
        ...
        if not self.args.no_minimization_of_mi_training:
            ...
            loss_h_s = (-1) * torch.log(torch.sum(torch.exp(style_content_pred_mi)[:, 0]) / len_) * (
                1 - torch.sum(labels).item() / len_
            ) - torch.log(torch.sum(torch.exp(style_content_pred_mi)[:, 1]) / len_) * (torch.sum(labels).item() / len_)
            ...
            R = torch.mean(
                (torch.exp(d_gamma_content_pred_u[:, 0]) / torch.exp(d_gamma_content_pred_v[:, 1]))
                ** (self.args.alpha - 1)
            )
            ...
            log_p_y_true_given_x = torch.masked_select(
                self.style_classifier(content_mi), torch.nn.functional.one_hot(labels).bool()
            )
            log_p_y_false_given_x = torch.masked_select(
                self.style_classifier(content_mi), ~(torch.nn.functional.one_hot(labels).bool())
            )
...
class RaoClassificationStyleEmdedding(torch.nn.Module):
    ...
    def __init__(self, args, reny_dataloader):
        ...
        self.downstream_classifier = Classifier(args.content_dim, args.number_of_styles, args.use_complex_classifier)
    ...
    def forward(self, input_tensor, protected_labels, downstream_labels):
        loss_mi, reny, loss_gamma, loss_style_classifier, loss_h_sz, loss_h_s, loss_adv_classifier = (
            torch.tensor(0.0).to(self.args.device),
            torch.tensor(0.0).to(self.args.device),
            torch.tensor(0.0).to(self.args.device),
            torch.tensor(0.0).to(self.args.device),
        ...
        gradient_encoder, gradient_content_proj, gradient_reny = (
            torch.tensor(0.0).to(self.args.device),
            torch.tensor(0.0).to(self.args.device),
            torch.tensor(0.0).to(self.args.device),
        )
        ...
        if self.training:
            ...
            loss.backward()
            gradient_encoder, gradient_content_proj = comput_gradient_norm(self.encoder), comput_gradient_norm(
                self.proj_content
            )
            torch.nn.utils.clip_grad_norm_(self.proj_content.parameters(), self.args.max_grad_norm)
            torch.nn.utils.clip_grad_norm_(self.encoder.parameters(), self.args.max_grad_norm)
        ...
        losses_dic = {
            "loss_down_classifier": loss_down_classifier,
            "loss_mi": loss_mi,
            "reny": reny,
            "loss_gamma": loss_gamma,
            "loss_h_sz": loss_h_sz,
...
class ClassificationStyleEmdedding(torch.nn.Module):
    ...
    def __init__(self, args, reny_dataloader):
        ...
        self.advers_classifier = Classifier(args.content_dim, args.number_of_styles, args.use_complex_classifier)
        self.downstream_classifier = Classifier(
            args.content_dim, args.number_of_styles, args.use_complex_classifier  # number_of_downstream_labels
        )
    ...
    def forward(self, input_tensor, protected_labels, downstream_labels):
        ...
        for step, batch in enumerate(self.reny_dataloader):
            ...
            if not self.args.no_minimization_of_mi_training:
                ...
                label_u = torch.tensor(
                    [[1.0, 0.0] if el == 0 else [0.0, 1.0] for el in labels_reny_protected.tolist()]
                ).to(self.args.device)
        ...
        if not self.args.no_minimization_of_mi_training:
            ...
            loss_h_s = (-1) * torch.log(torch.sum(torch.exp(adv_content_pred_mi)[:, 0]) / len_) * (
                1 - torch.sum(protected_labels).item() / len_
            ) - torch.log(torch.sum(torch.exp(adv_content_pred_mi)[:, 1]) / len_) * (
                torch.sum(protected_labels).item() / len_
            )
            ...
            label_u = torch.tensor([[1.0, 0.0] if el == 0 else [0.0, 1.0] for el in protected_labels.tolist()]).to(
                self.args.device
            )
            u = torch.cat([content_mi, label_u.unsqueeze(0).float().repeat(4, 1, 1)], dim=-1)
            v = torch.cat([content_mi, label_v.unsqueeze(0).float().repeat(4, 1, 1)], dim=-1)
            ...
            log_p_y_true_given_x = torch.masked_select(
                self.advers_classifier(content_mi), torch.nn.functional.one_hot(protected_labels).bool()
            )
            log_p_y_false_given_x = torch.masked_select(
                self.advers_classifier(content_mi), ~(torch.nn.functional.one_hot(protected_labels).bool())
            )
...
class KernelClassificationStyleEmdedding(torch.nn.Module):
    ...
    def __init__(self, args, kernel_dataloader):
        ...
        if self.args.mi_estimator == "KNIFE":
            self.conditionnal_entropy_kernel_1 = MultiGaussKernelEE(
                args,
                self.number_of_point_kernel,
                args.content_dim,
            ...
            self.conditionnal_entropy_kernel_2 = MultiGaussKernelEE(
                args,
                self.number_of_point_kernel,
                args.content_dim,
                # [K, d] to initialize the kernel :) so K is the number of points :)
            ...
            self.entropy_kernel = MultiGaussKernelEE(
                args,
                self.number_of_point_kernel * args.number_of_styles,
                args.content_dim,
                # [K, d] to initialize the kernel :) so K is the number of points :)
                average="weighted",  # un
    ...
    def forward(self, input_tensor, protected_labels, downstream_labels):
        ...
        for step, batch in enumerate(self.kernel_dataloader):
            ...
            loss_entropy_conditionnal_0 = self.conditionnal_entropy_kernel_1(
                torch.sum(content, dim=0)[(labels_reny_protected == 0).bool()]
            ) * (
                torch.sum((labels_reny_protected.long() == 0).bool()) / float(labels_reny_protected.size(0))
            )  # is there a scale pb ? are we sure we output h(Z|y)
            if self.training:
                loss_entropy_conditionnal_0.backward()
                torch.nn.utils.clip_grad_norm_(self.conditionnal_entropy_kernel_1.parameters(), self.args.max_grad_norm)
                self.args.optimizer.step()
            ...
            loss_entropy_conditionnal_1 = (
                self.conditionnal_entropy_kernel_2(torch.sum(content, dim=0)[(labels_reny_protected == 1).bool()])
                * (torch.sum((labels_reny_protected.long() == 1).bool()) / float(labels_reny_protected.size(0))).item()
            )  # is there a scale pb ? are we sure we output h(Z|y)
            if self.training:
                loss_entropy_conditionnal_1.backward()
                torch.nn.utils.clip_grad_norm_(self.conditionnal_entropy_kernel_2.parameters(), self.args.max_grad_norm)
                self.args.optimizer.step()
        ...
        h_maginal_1 = (
            self.conditionnal_entropy_kernel_1(torch.sum(content, dim=0)[(protected_labels == 0).bool()])
            * (torch.sum((protected_labels.long() == 0).bool()) / float(protected_labels.size(0))).item()
        )  # is there a scale pb ? are we sure we output h(Z|y)
        # un truc de fou furieux le float? wtf

        h_maginal_2 = (
            self.conditionnal_entropy_kernel_2(torch.sum(content, dim=0)[(protected_labels == 1).bool()])
            * (torch.sum((protected_labels.long() == 1).bool()) / float(protected_labels.size(0))).item()
        )  # is there a scale pb ? are we sure we output h(Z|y)
...
class MultiGaussKernelEE(nn.Module):
    def __init__(
        ...
        self.logC = torch.tensor([-self.d / 2 * np.log(2 * np.pi)]).to(self.args.device)
...
class MIClassificationStyleEmdedding(torch.nn.Module):
    ...
    def forward(self, input_tensor, protected_labels, downstream_labels):
        ...
        for step, batch in enumerate(self.parameter_dataloader):
            ...
            training_loss_mi = self.mi_estimator.learning_loss(
                torch.sum(content, 0), labels_reny_protected.unsqueeze(-1)
            )

            if self.training:
                training_loss_mi.backward()
                torch.nn.utils.clip_grad_norm_(self.mi_estimator.parameters(), self.args.max_grad_norm)
                self.args.optimizer.step()
