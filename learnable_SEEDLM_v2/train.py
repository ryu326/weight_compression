...
def main(opts):
    ...
    net = Learnable_SEEDLM(input_dim= opts.data_dim, u_length = opts.u_length) # 들어갈 변수 많으니 argument 받아서 잘 집어넣어주는 코드 짜보자 (hint: opt 이용)
    net = net.to(device)
        ...
        for i, (weight) in enumerate(train_dataloader):
            ...
            # if (total_iter % 50000 == 0):
                ...
                with torch.no_grad() :
                    ...
                    net_eval = Learnable_SEEDLM(input_dim= opts.data_dim) # 들어갈 변수 많으니 argument 받아서 잘 집어넣어주는 코드 짜보자 (hint: opt 이용)
...
def ddp_or_single_process(argvs):
    ...
    folder_name = '/'.join(opts.dataset_path.split('/')[-2:]) + f'/lmbda_{opts.lmbda}_u_length_{opts.u_length}_batch_size{opts.batch_size}_total_iter{opts.iter}_seed{opts.seed}'
