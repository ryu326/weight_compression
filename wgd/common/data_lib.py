...
def get_image_dataset_from_tfds(
    name,
    split,
    shuffle,
    repeat,
    ...
    with tf.device("/cpu:0"):
        ...
        if patchsize is not None:  # filter out imgs smaller than patchsize (if not using full-sized images)
            if "cifar" in name:
                ...
                dataset = dataset.filter(lambda x: check_image_size(x["image"], patchsize))
        dataset = dataset.map(
            lambda x: process_image(
                x["image"],
                crop=crop,
                patchsize=patchsize,
                normalize=normalize,
                image_channels=img_channels,
                augment=augment_imgs,
            )
        )
...
def get_image_dataset_from_glob(
    file_glob,
    shuffle,
    repeat,
    drop_remainder,
    ...
    with tf.device("/cpu:0"):
        ...
        if ".npy" in file_glob:  # reading numpy arrays directly instead of from images
            ...
            dataset = dataset.map(
                lambda x: process_image(x, crop=crop, patchsize=patchsize, normalize=normalize),
                num_parallel_calls=preprocess_threads,
            )
        else:
            dataset = dataset.map(
        ...
        if "mnist" in data_spec:  # Do extra processing for imgs
            dataset = dataset.map(
                lambda x: process_image(
                    ...
                    patchsize=kwargs.get("patchsize"),
                    normalize=True,
                    image_channels=1,
