        ...
        if self.config.optimizer_config.lr_decay:
            ...
            if opt_config.decay_type == "const":
                ...
                decay_fn = optax.linear_schedule(
                    init_value=learning_rate, end_value=learning_rate * decay_factor, transition_steps=decay_steps
                )
            ...
            schedule_fn = optax.join_schedules(
                schedules=[main_lr_fn, decay_fn], boundaries=[train_eval_config.num_train_steps - decay_steps]
            )
        else:
    ...
    def simple_train_eval_loop(self, train_eval_config, workdir):
        ...
        if jax.process_index() == 0:
            ...
            if config.get("profile"):
                hooks += [periodic_actions.Profile(num_profile_steps=5, logdir=train_dir)]
        ...
        with metric_writers.ensure_flushes(train_writer):
            ...
            while step < config.num_train_steps:
                ...
                with jax.profiler.StepTraceAnnotation("train", step_num=step):
                    batch = jax.tree_map(jnp.asarray, next(self.train_iter))
                    eval_train = (
                        config.eval_every_steps > 0 and (step + substeps) % config.eval_every_steps == 0
                    ) or is_last_step
                    state, train_metrics = self.p_train_step(state, batch, eval_train)

                # Quick indication that training is happening.
                logging.log_first_n(logging.WARNING, "Ran training step %d.", 3, step)
                for h in hooks:
                    h(step)
                ...
                if config.get("log_imgs_every_steps") and (step % config.log_imgs_every_steps == 0 or is_last_step):
                    ...
                    image_src = (
                        data_spec in common.data_lib.IMG_SOURCE_NAMES
                        or np.any([name in data_spec for name in common.data_lib.IMG_SOURCE_NAMES])
                        or len(jnp.shape(batch)) > 2
                    )
                    if image_src:
                        ...
                        scatter_img = self.make_scatter_img(
                            state.params["nu_x"][:num_pts_to_plot],
                ...
                if (
                    ...
                    with report_progress.timed("eval"):
                        metrics_list = []
                        if (
                            config.num_eval_steps is None
                        ):  # Then we will iterate through the full eval_ds (assumed finite)
                        ...
                        for eval_step, batch in enumerate(eval_iter):
                            ...
                            eval_steps_done += 1
                            val_size += len(batch)
                        metrics_list = jax.tree_map(
                            float, metrics_list
                        )  # Convert jnp.Array type to scalars, to make tf happy.
