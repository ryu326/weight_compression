...
def get_hyper():
    ...
    #                                   gaussian_noise_vars)
    ds = [10, 5, 2]
    ds = hyper.izip(*[hyper.sweep(f"{split}_data_config.data_dim", ds) for split in ("train", "eval")])
    # nu_support_sizes = [10, 100, 200, 1000, 2000]
    # nu_support_sizes = [10, 20, 50, 100, 300, 1000]
    nu_support_sizes = [1000, 500, 200, 100, 50, 20, 10]
    nu_support_sizes = hyper.sweep("model_config.nu_support_size", nu_support_sizes)
    ...
    hparam_cfgs = hyper.product(ds, nu_support_sizes, rd_lambdas, tseeds)
    return hparam_cfgs
