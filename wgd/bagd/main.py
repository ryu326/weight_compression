...
def estimate_rd(mu_x, mu_w, nu_x, nu_w, distort_type: str, rd_lambda):
    ...
    loss = jnp.sum(mu_w * phi)
    # Find \pi^* via \phi
    pi = jnp.exp(phi - scaled_C) * jnp.outer(
        mu_w, nu_w
    )  # [m, n]; this is the optimized joint distribution P_X Q*_{Y|X}.
...
class Experiment(BaseExperiment):
    ...
    def init_state(self, rng: PRNGKey):
        ...
        state = TrainState.create(apply_fn=None, params=params, tx_fn=self.get_optimizer)  # We don't use this for BA.
        return state
