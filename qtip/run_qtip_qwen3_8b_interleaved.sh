#!/bin/bash
##########################################################################
##  QTIP Qwen3-8B end-to-end per K (interleaved):
##    for K in {2,3,4,5,6}:
...
HESS="../Wparam_dataset/quip_hess/qwen3_8b_1024"
model_key="qwen3_8b"
exp_type="ft1"
ft_epochs=0
...
for K in "${K_VALUES[@]}"; do
    ...
    python3 "$NOTIFY_PY" "QTIP Qwen3-8B ${exp_type} K=${K}bit eval 완료" "$OUT_DIR" \
        2>&1 | tee -a "$LOG_FILE"
done

echo "QTIP Qwen3-8B ${exp_type} pipeline complete for K=${K_VALUES[*]}"
