    ...
    def __init__(self, config: MixtralConfig, layer_idx: Optional[int] = None):
        ...
        if f"{layer_idx}_q" not in config.quip_params["skip_list"]:
            ...
            self.q_proj = nn.Linear(
                self.hidden_size, self.num_heads * self.head_dim, dtype=config.torch_dtype, bias=False
            )
        ...
        if f"{layer_idx}_o" not in config.quip_params["skip_list"]:
            ...
            self.o_proj = nn.Linear(
                self.num_heads * self.head_dim, self.hidden_size, dtype=config.torch_dtype, bias=False
            )
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        td_x = config.quip_params["td_x"]//2 ## 8
        td_y = config.quip_params["td_y"] ## 16
        L = config.quip_params["L"]
        K = config.quip_params["K"]
        K_v_proj = config.quip_params.get("K_v_proj", K)
        V = config.quip_params["V"]
