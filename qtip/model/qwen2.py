    ...
    def __init__(self, config: Qwen2Config, layer_idx: Optional[int] = None):
        ...
        if f"{layer_idx}_q" not in config.quip_params["skip_list"]:
            ...
            self.q_proj = nn.Linear(
                self.hidden_size, self.num_heads * self.head_dim, dtype=config.torch_dtype, bias=True
            )
        ...
        if f"{layer_idx}_o" not in config.quip_params["skip_list"]:
            ...
            self.o_proj = nn.Linear(
                self.num_heads * self.head_dim, self.hidden_size, dtype=config.torch_dtype, bias=True
            )
