...
class LlamaMLP(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        if f"{layer_idx}_gate" not in config.quip_params["skip_list"]:
            ...
            self.gate_proj = nn.Linear(self.hidden_size, self.intermediate_size, dtype=config.torch_dtype, bias=False)
        ...
        if f"{layer_idx}_up" not in config.quip_params["skip_list"]:
            ...
            self.up_proj = nn.Linear(self.hidden_size, self.intermediate_size, dtype=config.torch_dtype, bias=False)
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        if f"{layer_idx}_q" not in config.quip_params["skip_list"]:
            ...
            self.q_proj = nn.Linear(
                self.hidden_size, self.num_heads * self.head_dim, dtype=config.torch_dtype, bias=config.attention_bias
            )
        ...
        if f"{layer_idx}_o" not in config.quip_params["skip_list"]:
            ...
            self.o_proj = nn.Linear(
                self.num_heads * self.head_dim, self.hidden_size, dtype=config.torch_dtype, bias=config.attention_bias
            )
