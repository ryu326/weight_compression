...
from transformers import AutoModelForCausalLM, AutoTokenizer, CLIPModel, AutoModel, AutoModelForImageClassification
...
def quantize_clip_decoder(layer, idx, cb, args, device, pre_orig_emb,
    ...
    # CLIP 모델에 맞춘 양자화 대상
    quant_order = []
    for thing in [('attention.attention.value', 'v', 'q', 'v', 'col'),
                  ('attention.attention.query', 'q', 'q', 'q', 'col'),
                  ('attention.attention.key', 'k', 'q', 'k', 'col'),
                  ('attention.output.dense', 'o', 'o', 'o', 'row'),
...
def main(args):
    ...
    quantize_layer_block(model.dinov2.encoder.layer, 'vision')
