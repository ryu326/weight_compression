...
import glog
import torch
from transformers import SiglipModel as OriSiglip
...
def main(args):
    ...
    model = SiglipModel.from_pretrained(args.base_model,
                                      torch_dtype='auto',
                                      low_cpu_mem_usage=True,
                                      config=model_config)
    orig_model = OriSiglip.from_pretrained(args.base_model,
                                           torch_dtype='auto',
                                           low_cpu_mem_usage=True,
                                           config=model_config)
    ...
    copy_submodule(model.text_model.head, orig_model.text_model.head, 'text_projection')
    if hasattr(model.vision_model, "head") and hasattr(orig_model.vision_model, "head"):
        if hasattr(model.vision_model.head, "mlp") and hasattr(orig_model.vision_model.head, "mlp"):
            model.vision_model.head.mlp = orig_model.vision_model.head.mlp
