...
def main(args):
    ...
    ## complexity test
    # Convert weights to float16 in-place for parameters (do not assign tensors to .weight)
    model.model.embed_tokens.weight.data = model.model.embed_tokens.weight.data.to(torch.float16)
    model.model.norm.weight.data = model.model.norm.weight.data.to(torch.float16)
    model.lm_head.weight.data = model.lm_head.weight.data.to(torch.float16)
    torch.save(model.state_dict(), args.hf_output_path)
